  ...
  public Router setup(Vertx vertx) {
    ...
    router
        ...
        .blockingHandler(
            ctx -> {
              RequestBody reqBody = ctx.body();
              ListThreatApiRequest req =
                  ProtoMessageUtils.<ListThreatApiRequest>toProtoMessage(
                          ListThreatApiRequest.class, reqBody.asString())
                      .orElse(null);
              ...
              ProtoMessageUtils.toString(dsService.listThreatApis(ctx.get("accountId"), req))
                  .ifPresent(s -> ctx.response().setStatusCode(200).end(s));
            });
