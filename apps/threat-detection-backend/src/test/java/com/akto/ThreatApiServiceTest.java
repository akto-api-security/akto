 ...
 * Tests all three main methods: listThreatApis, getSubCategoryWiseCount, and
 * getSeverityWiseCount
...
public class ThreatApiServiceTest {
         ...
         * Verifies that the method correctly processes a simple request and returns
         * expected results
         */
        @Test
        ...
        void testListThreatApis_WithMethodAndUrlFilters() {
                ...
                // Note: In a real implementation, you might want to verify the actual pipeline
                // structure
                // by capturing the arguments passed to aggregate() and checking the filter
                // conditions
        }

        /**
         * Test listThreatApis with time range filter
         * This test is structured to work even though TimeRange might not be available
         * in proto
         */
        @Test
        @DisplayName("listThreatApis - With time range filter (if supported)")
        void testListThreatApis_WithTimeRangeFilter() {
                // Arrange: Create a basic filter (time range testing depends on proto
                // structure)
                ListThreatApiRequest.Filter filter = ListThreatApiRequest.Filter.newBuilder()
                                .addMethods("GET")
                                .build();
        ...
        void testListThreatApis_ExceptionHandling() {
                ...
                // The service should still return a valid response object even when exceptions
                // occur
        ...
        void testGetSubCategoryWiseCount_BasicFunctionality() {
                ...
                // Act
                ThreatCategoryWiseCountResponse response = threatApiService.getSubCategoryWiseCount(TEST_ACCOUNT_ID,
                                request);
        ...
        void testGetSeverityWiseCount_AllSeverities() {
                ...
                // Mock count results for each severity level (service checks in order:
                ...
                // Act
                ThreatSeverityWiseCountResponse response = threatApiService.getSeverityWiseCount(TEST_ACCOUNT_ID,
                                request);
                ...
                // Verify that countDocuments was called exactly 4 times (once for each
                // severity)
         ...
         * Creates documents that match the structure expected by the aggregation
         * pipeline
         * 
