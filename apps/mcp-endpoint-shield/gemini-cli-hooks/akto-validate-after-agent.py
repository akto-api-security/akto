...
if not logger.handlers:
    file_handler = logging.FileHandler(os.path.join(LOG_DIR, "validate-after-agent.log"))
...
WARN_STATE_PATH = os.path.join(LOG_DIR, "akto_after_agent_warn_pending.json")
...
def main():
    logger.info(f"=== AfterAgent hook started - Mode: {MODE}, Sync: {AKTO_SYNC_MODE} ===")
    ...
    if hook_event and hook_event != "AfterAgent":
        sys.exit(0)

    user_prompt = (input_data.get("prompt") or "").strip()
    response_text = (input_data.get("prompt_response") or "").strip()
    ...
    if not user_prompt or not response_text:
        logger.info("No complete interaction present, skipping")
        sys.exit(0)

    logger.info(
        f"Extracted interaction - Prompt: {len(user_prompt)} chars, "
        f"Response: {len(response_text)} chars, stop_hook_active: {stop_hook_active}"
    )

    if stop_hook_active:
        logger.info("stop_hook_active=true: skipping guardrails block to avoid retry loops")
    ...
    if AKTO_SYNC_MODE and not stop_hook_active:
        ...
        if not allowed:
            ...
            # gemini-cli's AfterAgent docs: stderr becomes the feedback prompt on retry
            sys.stderr.write(f"Blocked by Akto Guardrails: {gr_reason}\n")
