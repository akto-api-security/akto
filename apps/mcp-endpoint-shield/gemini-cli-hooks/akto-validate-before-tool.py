...
if not logger.handlers:
    file_handler = logging.FileHandler(os.path.join(LOG_DIR, "validate-before-tool.log"))
...
def build_validation_request(
    ...
    tags = {"gen-ai": "Gen AI", "tool-use": "PreTool", "tool_name": tool_name}
    if MODE == "atlas":
...
def call_guardrails(
    ...
    if not AKTO_DATA_INGESTION_URL:
        logger.warning("AKTO_DATA_INGESTION_URL not set, allowing tool call (fail-open)")
        return True, ""
    ...
    try:
        ...
        if allowed:
            ...
            logger.warning(f"Tool request DENIED for {tool_name}: {reason}")
...
def main():
    ...
    logger.info(f"Processing tool request: {tool_name}")

    if AKTO_SYNC_MODE:
        allowed, reason = call_guardrails(tool_name, tool_input, session_info)
        if not allowed:
