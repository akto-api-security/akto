...
if not logger.handlers:
    file_handler = logging.FileHandler(os.path.join(LOG_DIR, "validate-before-agent.log"))
...
WARN_STATE_PATH = os.path.join(LOG_DIR, "akto_before_agent_warn_pending.json")
...
def build_validation_request(query: str, session_info: dict = None) -> dict:
    ...
    response_headers = json.dumps({"x-gemini-hook": "BeforeAgent"})
...
def main():
    logger.info(f"=== BeforeAgent hook started - Mode: {MODE}, Sync: {AKTO_SYNC_MODE} ===")
    ...
    if hook_event and hook_event != "BeforeAgent":
        sys.exit(0)
