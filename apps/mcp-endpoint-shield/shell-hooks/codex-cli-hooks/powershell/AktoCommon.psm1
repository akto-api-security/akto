# AktoCommon.psm1 - shared helpers for Akto codex-cli hooks (PowerShell port).
#
...
$script:LogDir   = if ($env:LOG_DIR) { $env:LOG_DIR } else { Join-Path $HOME ".codex/akto/logs" }
...
$script:Connector = if ($env:AKTO_CONNECTOR) { $env:AKTO_CONNECTOR } else { "codex_cli" }
$script:ConnectorValue = if ($env:AKTO_CONNECTOR_VALUE) { $env:AKTO_CONNECTOR_VALUE } else { "codexcli" }
