#!/usr/bin/env bash
# akto-validate-post-tool.sh - bash port of codex akto-validate-post-tool.py (PostToolUse)
set -u
...
if [ "$AKTO_MODE" = "atlas" ]; then
  if [ -n "$DEVICE_ID" ]; then CODEX_API_HOST="https://${DEVICE_ID}.ai-agent.${AKTO_CONNECTOR_VALUE}"; else CODEX_API_HOST="$CODEX_DETECTED_HOST"; fi
else
...
build_ingestion_payload() {  # TOOL_NAME TOOL_INPUT_RAW TOOL_RESPONSE_RAW
  ...
  if [ "$IS_MCP" = "1" ]; then
    ...
    tags="{\"gen-ai\":\"Gen AI\",\"ai-agent\":$(json_str "$AKTO_CONNECTOR_VALUE"),\"tool-use\":\"Tool Execution\",\"tool_name\":$(json_str "$tool_name")"
  ...
  tags="$tags}"
  req_hdr="{\"host\":$(json_str "$host"),\"x-codex-hook\":\"PostToolUse\",\"content-type\":\"application/json\""
  ...
  local response_headers; response_headers="$(json_str '{"x-codex-hook":"PostToolUse","content-type":"application/json"}')"
...
ingest_blocked() {  # TOOL_NAME TOOL_INPUT_RAW TOOL_RESPONSE_RAW
  [ -z "$AKTO_DATA_INGESTION_URL" ] && return 0
  [ "$AKTO_SYNC_MODE" != "true" ] && return 0
  should_ingest_nonmcp || { log_info "Skipping non-MCP blocked PostToolUse ingestion (set AKTO_INGEST_NON_MCP_TOOLS=true to re-enable)"; return 0; }
  local body; body="$(build_ingestion_payload "$1" "$2" "$3")"
...
main() {
  ...
  local tool_name tool_input tool_response tool_use_id
  ...
  tool_use_id="$(json_string "$input" tool_use_id 2>/dev/null)"
  parse_codex_tool "$tool_name"

  if [ "$IS_MCP" = "1" ]; then
    log_info "Processing MCP tool response: $tool_name (server=$MCP_SERVER, mcpTool=$MCP_TOOL)"
  else
    log_info "Processing non-MCP tool response: $tool_name"
  fi
  ...
  if [ "$AKTO_SYNC_MODE" = "true" ]; then
    ...
    if [ -n "$tool_input" ] && [ -n "$tool_response" ]; then
      ...
      local fp; fp="$(printf '{"i":%s,"r":%s,"t":%s,"u":%s}' "$tool_input" "$tool_response" "$(json_str "$tool_name")" "$(json_str "$tool_use_id")" | sha256_hex)"
      ...
      if [ "$ALLOWED" = "false" ]; then
        ...
        ingest_blocked "$tool_name" "$tool_input" "$tool_response"
        exit 0
      fi
    fi
  fi
