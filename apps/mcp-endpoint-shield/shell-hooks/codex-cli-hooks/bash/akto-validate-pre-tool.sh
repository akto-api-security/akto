#!/usr/bin/env bash
# akto-validate-pre-tool.sh - bash port of codex akto-validate-pre-tool.py (PreToolUse)
# Validates Bash / apply_patch / MCP tool requests; emits a PreToolUse permissionDecision.
set -u
...
# parse_codex_tool TOOLNAME -> sets IS_MCP(0/1) MCP_SERVER MCP_TOOL
parse_codex_tool() {
...
normalize_tool_name() {  # -> normalized url path segment (url-safe chars)
...
build_validation_request() {  # TOOL_NAME TOOL_INPUT_RAW
  ...
  if [ "$IS_MCP" = "1" ]; then
    ...
    tags="{\"gen-ai\":\"Gen AI\",\"ai-agent\":$(json_str "$AKTO_CONNECTOR_VALUE"),\"tool_name\":$(json_str "$tool_name")"
  ...
  req_hdr="{\"host\":$(json_str "$host"),\"x-codex-hook\":\"PreToolUse\",\"content-type\":\"application/json\""
  ...
  local response_headers; response_headers="$(json_str '{"x-codex-hook":"PreToolUse"}')"
...
ingest_blocked() {  # TOOL_NAME TOOL_INPUT_RAW REASON
  ...
  if [ "$IS_MCP" != "1" ] && [ "$AKTO_INGEST_NON_MCP_TOOLS" != "true" ]; then
    log_info "Skipping non-MCP blocked-request ingestion (set AKTO_INGEST_NON_MCP_TOOLS=true to re-enable)"; return 0
  fi
...
main() {
  ...
  if [ "$IS_MCP" = "1" ]; then
    ...
    log_info "Processing non-MCP tool request: $tool_name"
  fi
  ...
  if [ "$AKTO_SYNC_MODE" = "true" ]; then
    ...
    if [ "$ALLOWED" = "false" ]; then
      ...
      ingest_blocked "$tool_name" "$tool_input" "${GR_REASON:-Policy violation}"
      exit 0
    fi
  fi
