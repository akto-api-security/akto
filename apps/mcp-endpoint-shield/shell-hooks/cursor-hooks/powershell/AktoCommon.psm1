# AktoCommon.psm1 - shared helpers for Akto cursor hooks (PowerShell port).
#
...
$script:LogDir   = if ($env:LOG_DIR) { $env:LOG_DIR } else { Join-Path $HOME ".cursor/akto/chat-logs" }
...
$script:Connector = if ($env:AKTO_CONNECTOR) { $env:AKTO_CONNECTOR } else { "cursor" }
$script:ConnectorValue = if ($env:AKTO_CONNECTOR_VALUE) { $env:AKTO_CONNECTOR_VALUE } else { "cursor" }
