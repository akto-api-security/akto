...
# ---------------------------------------------------------------------------
# get_device_ip (argus) - local LAN IP of primary outbound interface.
# Mirrors akto_helpers.get_device_ip (UDP 8.8.8.8 trick); bash best-effort.
# ---------------------------------------------------------------------------
get_device_ip() {
  [ -n "${_AKTO_DEVICE_IP:-}" ] && { printf '%s' "$_AKTO_DEVICE_IP"; return 0; }
  local ip=""
  case "$_akto_uname" in
    Darwin)
      ip="$(route get 8.8.8.8 2>/dev/null | awk '/interface:/{print $2}' | while read -r i; do ipconfig getifaddr "$i" 2>/dev/null; done | head -1)"
