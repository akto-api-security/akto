#!/usr/bin/env bash
# akto_common.sh - shared helpers for Akto gemini-cli hooks (bash port).
#
...
# ---------------------------------------------------------------------------
# Config (env with defaults) - mirrors the Python gemini hooks 1:1
# ---------------------------------------------------------------------------
AKTO_LOG_DIR="${LOG_DIR:-$HOME/.gemini/akto/chat-logs}"
...
AKTO_CONNECTOR="${AKTO_CONNECTOR:-gemini_cli}"
AKTO_CONNECTOR_VALUE="${AKTO_CONNECTOR_VALUE:-geminicli}"
...
#   warn_pending_has FILE FP   -> return 0 if present
#   warn_pending_add FILE FP / warn_pending_del FILE FP
