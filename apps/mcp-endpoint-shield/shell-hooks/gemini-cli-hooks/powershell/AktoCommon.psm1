# AktoCommon.psm1 - shared helpers for Akto gemini-cli hooks (PowerShell port).
#
...
$script:LogDir   = if ($env:LOG_DIR) { $env:LOG_DIR } else { Join-Path $HOME ".gemini/akto/chat-logs" }
...
$script:Connector = if ($env:AKTO_CONNECTOR) { $env:AKTO_CONNECTOR } else { "gemini_cli" }
$script:ConnectorValue = if ($env:AKTO_CONNECTOR_VALUE) { $env:AKTO_CONNECTOR_VALUE } else { "geminicli" }
...
function Get-AktoConfig { $script:Mode }
...
function Get-AktoConnectorValue { $script:ConnectorValue }
function Get-AktoMcpIngestPath { $script:McpIngestPath }
function Get-AktoNonMcpPrefix { $script:NonMcpPrefix }
