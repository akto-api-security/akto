        ...
        if command -v jq >/dev/null 2>&1; then
            jq --argjson newhooks "$new_hooks_config" '
                .hooks.UserPromptSubmit = (
                    [(.hooks.UserPromptSubmit // [])[] | select(.hooks | map(.command) | any(contains("akto")) | not)] +
                    $newhooks.hooks.UserPromptSubmit
                ) |
                .hooks.Stop = (
                    [(.hooks.Stop // [])[] | select(.hooks | map(.command) | any(contains("akto")) | not)] +
                    $newhooks.hooks.Stop
                )
...
new_hooks = json.loads('''$new_hooks_config''')
if 'hooks' not in settings:
    settings['hooks'] = {}
for event in ('UserPromptSubmit', 'Stop'):
    existing = settings.get('hooks', {}).get(event, [])
    filtered = [e for e in existing if not any('akto' in h.get('command', '') for h in e.get('hooks', []))]
    settings['hooks'][event] = filtered + new_hooks['hooks'][event]
with open('$CLAUDE_SETTINGS_FILE', 'w') as f:
    json.dump(settings, f, indent=2)
" 2>&1
            log "Merged hooks into existing settings.json (via python3)"
        else
            log_error "Neither jq nor python3 available — cannot safely merge into existing settings.json"
            log_error "Skipping settings.json update to avoid destroying existing configuration"
            return 1
        fi
    else
