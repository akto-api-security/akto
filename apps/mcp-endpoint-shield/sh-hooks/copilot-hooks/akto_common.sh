...
get_connector_config() {
  ...
  CFG_AI_AGENT_TAG="copilot"
  CFG_ATLAS_DOMAIN="ai-agent.copilot"
  if [[ "$connector" == "vscode" ]]; then
    CFG_IS_VSCODE="true"
...
parse_github_tool() {
  ...
  if [[ "$tool_name" == mcp_* ]]; then
    local rest="${tool_name#mcp_}"
    local server="${rest%%_*}"
    local mcp_tool="${rest#*_}"
    if [[ -n "$server" && -n "$mcp_tool" && "$mcp_tool" != "$rest" ]]; then
      printf 'true\t%s\t%s\n' "$server" "$mcp_tool"
      return
    fi
  fi
  if [[ "$tool_name" == *-* ]]; then
    local server="${tool_name%-*}"
    local mcp_tool="${tool_name##*-}"
    if [[ -n "$server" && -n "$mcp_tool" ]]; then
      printf 'true\t%s\t%s\n' "$server" "$mcp_tool"
      return
    fi
  fi
...
build_tools_call_jsonrpc() {
  local mcp_tool_name="$1" tool_input="$2"
  jq -n -c --arg name "$mcp_tool_name" --argjson input "$tool_input" '
    {jsonrpc:"2.0", method:"tools/call",
     params:{name:$name, arguments:(if ($input|type)=="object" then $input elif $input==null then {} else {input:$input} end)},
     id:1}
  '
}
...
build_tools_call_result_jsonrpc() {
  ...
  jq -n -c --argjson resp "$tool_response" '
    {jsonrpc:"2.0", id:1, result:(if ($resp|type)=="object" then $resp else {output:$resp} end)}
  '
}
...
_AKTO_ID_FIELDS_JSON='["session_id","cwd","hook_event_name"]'
...
current_message_turn() {
  local input_json="$1" session_info state_key row
  session_info=$(_akto_extract_session_info "$input_json")
  state_key=$(_akto_state_key "$input_json" "$session_info")
  row=$(_akto_load_session_state "$state_key")
  jq -r '.current_message_id // empty' <<<"$row"
}
...
get_last_turn() {
  ...
  local last_user="" last_user_idx=-1 idx total
  total=$(wc -l < "$path" | tr -d ' ')
  idx=$total
  while IFS= read -r line; do
    [[ -z "$line" ]] && { idx=$((idx - 1)); continue; }
    local etype content
    etype=$(jq -r '.type // ""' 2>/dev/null <<<"$line")
    if [[ "$etype" == "user.message" ]]; then
      content=$(_akto_extract_copilot_content <<<"$line")
      if [[ -n "$content" ]]; then
        last_user="$content"
        last_user_idx=$idx
        idx=$((idx - 1))
  ...
  if [[ $last_user_idx -eq -1 ]]; then
    printf '\t'
    return
  fi

  local response="" cur=0
  while IFS= read -r line; do
    cur=$((cur + 1))
    [[ $cur -le $last_user_idx ]] && continue
    [[ -z "$line" ]] && continue
    local etype content
    etype=$(jq -r '.type // ""' 2>/dev/null <<<"$line")
    if [[ "$etype" == "assistant.message" ]]; then
      content=$(_akto_extract_copilot_content <<<"$line")
      [[ -n "$content" ]] && response="${response}${content}"
    fi
  done < "$path"

  printf '%s\t%s' "$last_user" "$response"
}

_akto_heartbeat_agent_id() {
  local log_dir="$1" path="$log_dir/agent_id" id
  if [[ -f "$path" ]]; then
    id=$(cat "$path" 2>/dev/null)
    [[ -n "$id" ]] && { printf '%s' "$id"; return; }
  fi
  id="$(date +%s%N)"
  printf '%s' "$id" > "$path" 2>/dev/null
  printf '%s' "$id"
}

_akto_heartbeat_should_send() {
  local log_dir="$1" path="$log_dir/last_heartbeat" last now
  [[ -f "$path" ]] || return 0
  last=$(cat "$path" 2>/dev/null)
  [[ -z "$last" ]] && return 0
  now=$(date +%s)
  (( now - ${last%.*} < 30 )) && return 1
  return 0
}

send_heartbeat() {
  local log_dir="$1" logfile="${2:-hook-executions.log}"
  mkdir -p "$log_dir" 2>/dev/null
  _akto_heartbeat_should_send "$log_dir" || return 0

  local db_url device_id username agent_id now_s payload
  db_url="${DATABASE_ABSTRACTOR_SERVICE_URL:-https://cyborg.akto.io}"
  db_url="${db_url%/}"
  device_id="${DEVICE_ID:-$(get_machine_id)}"
  username="$(get_username)"
  agent_id="$(_akto_heartbeat_agent_id "$log_dir")"
  now_s=$(date +%s)

  payload=$(jq -n -c \
    --arg id "$agent_id" --arg name "$device_id" --arg user "$username" --arg ts "$now_s" '
    {moduleInfo: {
      id: $id, name: $name, moduleType: "MCP_ENDPOINT_SHIELD", currentVersion: "1.0.0",
      startedTs: ($ts|tonumber), lastHeartbeatReceived: ($ts|tonumber),
      additionalData: {username: $user, mcpServers: {}}
    }}
  ')

  local hdrs=(-H "Content-Type: application/json")
  [[ -n "${AKTO_API_TOKEN:-}" ]] && hdrs+=(-H "Authorization: ${AKTO_API_TOKEN}")
  if curl -sS -k --max-time 3 -X POST "${hdrs[@]}" --data-binary "$payload" \
      -o /dev/null "$db_url/api/updateModuleInfoForHeartbeat" 2>/dev/null; then
    printf '%s' "$now_s" > "$log_dir/last_heartbeat" 2>/dev/null
    log_info "$logfile" "Heartbeat sent: agentId=$agent_id, deviceId=$device_id, username=$username"
  fi
}
