...
_AKTO_ID_FIELDS_JSON='["session_id","conversation_id","generation_id","model","transcript_path","user_email","cwd","permission_mode","hook_event_name"]'
...
_akto_extract_codex_text() {
  jq -r '
    . as $c
    | if ($c|type)=="string" then ($c | gsub("^\\s+|\\s+$";""))
      elif ($c|type)=="array" then
        (([$c[] | select(type=="object" and (.type=="input_text" or .type=="output_text" or .type=="text")) | .text // ""]) | join("")) | gsub("^\\s+|\\s+$";"")
      else "" end
  ' 2>/dev/null
}
...
get_last_user_prompt() {
  ...
  while IFS= read -r line; do
    [[ -z "$line" ]] && continue
    local etype ptype prole content text
    etype=$(jq -r '.type // ""' 2>/dev/null <<<"$line") || continue
    [[ "$etype" != "response_item" ]] && continue
    ptype=$(jq -r '.payload.type // ""' <<<"$line")
    prole=$(jq -r '.payload.role // ""' <<<"$line")
    if [[ "$ptype" == "message" && "$prole" == "user" ]]; then
      content=$(jq -c '.payload.content // []' <<<"$line")
      text=$(_akto_extract_codex_text <<<"$content")
      [[ -n "$text" ]] && last="$text"
    fi
  done < "$path"
  printf '%s' "$last"
