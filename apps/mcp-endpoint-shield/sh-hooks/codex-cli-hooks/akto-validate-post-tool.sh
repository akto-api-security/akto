...
build_hook_tags() {
  ...
  jq -n -c --arg mode "$MODE" --arg src "$CONTEXT_SOURCE" --arg av "$AKTO_CONNECTOR_VALUE" --arg tn "$tool_name" --argjson is_mcp "$is_mcp" '
    (if $is_mcp then {"mcp-server":"MCP Server","mcp-client":$av}
     else {"gen-ai":"Gen AI","ai-agent":$av,"tool-use":"Tool Execution","tool_name":$tn} end)
...
build_ingestion_payload() {
  ...
  local tags host req_headers installer_hdrs request_payload response_payload path now_ms
  ...
  req_headers=$(jq -n -c --arg host "$host" --arg mcps "$mcp_server" --argjson is_mcp "$is_mcp" --argjson inst "$installer_hdrs" '
    {"host":$host, "x-codex-hook":"PostToolUse", "content-type":"application/json"}
  ...
  jq -n -c \
    --arg path "$path" --argjson reqh "$req_headers" --argjson reqp "$request_payload" --argjson resp "$response_payload" \
    --arg ip "$(get_username)" --arg time "$now_ms" --argjson tag "$tags" --arg src "$CONTEXT_SOURCE" '
    {
      path: $path,
      requestHeaders: ($reqh|tojson),
      responseHeaders: ({"x-codex-hook":"PostToolUse","content-type":"application/json"}|tojson),
      method: "POST",
...
call_guardrails() {
  ...
  if [[ -z "$AKTO_DATA_INGESTION_URL" ]]; then
    log_warn "$LOGFILE" "AKTO_DATA_INGESTION_URL not set, allowing tool result (fail-open)"
    jq -n -c '{allowed:true,reason:"",behaviour:""}'
    return
  fi

  if [[ "$is_mcp" == "true" ]]; then
    log_info "$LOGFILE" "Validating MCP tools/call result for $mcp_tool (server=$mcp_server, codexTool=$tool_name)"
  else
    log_info "$LOGFILE" "Validating PostToolUse result for: $tool_name"
  fi
  ...
  if [[ "$(jq -r '.allowed' <<<"$parsed")" == "true" ]]; then
    log_info "$LOGFILE" "Tool result ALLOWED by guardrails for $tool_name"
  else
    log_warn "$LOGFILE" "Tool result DENIED by guardrails for $tool_name: $(jq -r '.reason' <<<"$parsed")"
  fi
  printf '%s' "$parsed"
}

posttool_fingerprint() {
  local tool_name="$1" tool_use_id="$2" tool_input_json="$3" tool_response_json="$4"
  jq -n -c -S --arg t "$tool_name" --arg u "${tool_use_id:-}" --argjson i "$tool_input_json" --argjson r "$tool_response_json" \
    '{t:$t,u:$u,i:$i,r:$r}' | _akto_sha256
}
...
ingest_blocked_tool_result() {
  ...
  if [[ "$is_mcp" != "true" && "$AKTO_INGEST_NON_MCP_TOOLS" != "true" ]]; then
    log_info "$LOGFILE" "Skipping non-MCP blocked PostToolUse ingestion (set AKTO_INGEST_NON_MCP_TOOLS=true to re-enable)"
    return
  fi

  log_info "$LOGFILE" "Ingesting blocked PostToolUse data"
  local req_body url
  req_body=$(build_ingestion_payload "$tool_name" "$tool_input_json" "$tool_response_json" "$is_mcp" "$mcp_server" "$mcp_tool" "$session_info_json")
  req_body=$(jq -c --arg reason "${reason:-Policy violation}" '
    .responseHeaders = ({"x-codex-hook":"PostToolUse","x-blocked-by":"Akto Proxy","content-type":"application/json"} | tojson)
  ...
  if akto_post_json "$url" "$req_body" "$LOGFILE" >/dev/null; then
    log_info "$LOGFILE" "Blocked PostToolUse ingestion successful"
  else
...
send_ingestion_data() {
  ...
  if [[ "$is_mcp" == "true" ]]; then
    log_info "$LOGFILE" "Ingesting MCP tools/call result for $mcp_tool (server=$mcp_server, codexTool=$tool_name)"
  else
    log_info "$LOGFILE" "Ingesting tool response for: $tool_name"
  fi
...
main() {
  log_info "$LOGFILE" "=== PostToolUse hook started - Mode: $MODE, Sync: $AKTO_SYNC_MODE ==="
  ...
  local tool_name tool_input_json tool_response_json tool_use_id mcp_json is_mcp mcp_server mcp_tool session_id
  ...
  tool_use_id=$(jq -r '.tool_use_id // ""' <<<"$input_data")
  ...
  if [[ "$is_mcp" == "true" ]]; then
    log_info "$LOGFILE" "Session: $session_id, Processing MCP tool response: $tool_name (server=$mcp_server, mcpTool=$mcp_tool)"
  else
    log_info "$LOGFILE" "Session: $session_id, Processing non-MCP tool response: $tool_name"
  fi
  ...
  if [[ "$AKTO_SYNC_MODE" == "true" ]]; then
    ...
    fingerprint=$(posttool_fingerprint "$tool_name" "$tool_use_id" "$tool_input_json" "$tool_response_json")
    ...
    if [[ "$allowed" != "true" ]]; then
      ...
      ingest_blocked_tool_result "$tool_name" "$tool_input_json" "$tool_response_json" "$gr_reason" "$is_mcp" "$mcp_server" "$mcp_tool" "$session_info"
      exit 0
    fi
  fi
