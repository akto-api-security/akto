...
build_validation_request() {
  ...
  local device_id host installer_hdrs req_headers now_ms
  ...
  jq -n -c \
    ...
    | ({"host":$host,"x-claude-hook":"UserPromptSubmit","content-type":"application/json"} + $inst) as $reqh
    | {
        path: "/v1/messages",
        requestHeaders: ($reqh|tojson),
        responseHeaders: ({"x-claude-hook":"UserPromptSubmit"}|tojson),
        method: "POST",
...
ingest_blocked_request() {
  ...
  req_body=$(jq -c --arg reason "${reason:-Policy violation}" '
    .responseHeaders = ({"x-claude-hook":"UserPromptSubmit","x-blocked-by":"Akto Proxy","content-type":"application/json"} | tojson)
