...
LOGFILE="validate-mcp-request.log"
...
build_validation_request() {
  ...
  local tags host req_hdr installer_hdrs request_payload path now_ms
  ...
  req_hdr=$(jq -n -c --arg host "$host" --arg mcps "$mcp_server" --argjson is_mcp "$is_mcp" --argjson inst "$installer_hdrs" '
    {"host":$host, "x-claude-hook":"PreToolUse", "content-type":"application/json"}
  ...
  jq -n -c \
    --arg path "$path" --argjson reqh "$req_hdr" --argjson reqp "$request_payload" \
    --arg ip "$(get_username)" --arg time "$now_ms" --argjson tag "$tags" --arg src "$CONTEXT_SOURCE" '
    {
      path: $path,
      requestHeaders: ($reqh|tojson),
      responseHeaders: ({"x-claude-hook":"PreToolUse"}|tojson),
      method: "POST",
...
call_guardrails() {
  ...
  if [[ "$(jq -r '(. == {} or . == null or . == [] or . == "" or . == false or . == 0)' <<<"$tool_input_json")" == "true" ]]; then
    jq -n -c '{allowed:true,reason:"",behaviour:"",modified:false,modified_payload:""}'
    return
  fi
  ...
  if [[ "$is_mcp" == "true" ]]; then
    log_info "$LOGFILE" "Validating MCP tools/call for $mcp_tool (server=$mcp_server, claudeTool=$tool_name)"
  else
  ...
  parsed=$(jq -c '
    ...
    | {
        allowed: (if ($gr.Allowed == null) then true else $gr.Allowed end),
        reason: ($gr.Reason // ""),
        behaviour: ($gr.behaviour // $gr.Behaviour // ""),
        modified: (if ($gr.Modified == null) then false else $gr.Modified end),
        modified_payload: ($gr.ModifiedPayload // "")
      }
  ' <<<"$result" 2>/dev/null)
  if [[ -z "$parsed" ]]; then
    jq -n -c '{allowed:true,reason:"",behaviour:"",modified:false,modified_payload:""}'
...
extract_tool_input_from_modified_payload() {
  local modified_payload_raw="$1" is_mcp="$2" fallback_json="$3"
  if [[ -z "$modified_payload_raw" ]]; then
    printf '%s' "$fallback_json"; return
  fi
  if [[ -z "${modified_payload_raw//[$'\t\r\n ']/}" ]]; then
    printf '%s' "$fallback_json"; return
  fi

  local parsed
  parsed=$(jq -c '.' <<<"$modified_payload_raw" 2>/dev/null)
  if [[ -z "$parsed" ]]; then
    log_warn "$LOGFILE" "ModifiedPayload is not valid JSON; keeping original tool_input"
    printf '%s' "$fallback_json"; return
  fi
  if [[ "$(jq -r 'type' <<<"$parsed")" != "object" ]]; then
    printf '%s' "$fallback_json"; return
  fi

  if [[ "$is_mcp" == "true" ]]; then
    local args
    args=$(jq -c '.params.arguments // null' <<<"$parsed")
    if [[ "$(jq -r 'type' <<<"$args")" == "object" ]]; then
      printf '%s' "$args"
    else
      log_warn "$LOGFILE" "MCP ModifiedPayload missing params.arguments dict; keeping original tool_input"
  ...
  local body
  body=$(jq -c '.body // null' <<<"$parsed")
  if [[ "$(jq -r 'type' <<<"$body")" == "object" ]]; then
    printf '%s' "$body"
  else
    log_warn "$LOGFILE" "Non-MCP ModifiedPayload missing body dict; keeping original tool_input"
    printf '%s' "$fallback_json"
  fi
}

pretool_allow_output() {
  local updated_input_json="${1:-null}" reason="${2:-Tool request allowed}"
  jq -n -c --arg r "$reason" --argjson upd "$updated_input_json" '
    {hookSpecificOutput: ({hookEventName:"PreToolUse", permissionDecision:"allow", permissionDecisionReason:$r}
      + (if $upd != null then {updatedInput:$upd} else {} end))}
  '
}
...
ingest_blocked_request() {
  ...
  req_body=$(jq -c --arg reason "${reason:-Policy violation}" '
    .responseHeaders = ({"x-claude-hook":"PreToolUse","x-blocked-by":"Akto Proxy","content-type":"application/json"} | tojson)
...
main() {
  ...
  local tool_name tool_input_json mcp_json is_mcp mcp_server mcp_tool
  ...
  local modified="false" modified_payload=""

  if [[ "$is_mcp" == "true" ]]; then
    log_info "$LOGFILE" "Processing MCP tool request: $tool_name (server=$mcp_server, mcpTool=$mcp_tool)"
  else
    log_info "$LOGFILE" "Processing non-MCP tool request (gen-ai only): $tool_name"
  fi
  ...
  if [[ "$AKTO_SYNC_MODE" == "true" ]]; then
    ...
    modified=$(jq -r '.modified' <<<"$gr_json")
    modified_payload=$(jq -r '.modified_payload' <<<"$gr_json")
    ...
    if [[ "$allowed" != "true" ]]; then
      ...
      ingest_blocked_request "$tool_name" "$tool_input_json" "$gr_reason" "$is_mcp" "$mcp_server" "$mcp_tool" "$session_info"
      exit 0
    fi
  fi

  if [[ "$modified" == "true" && -n "$modified_payload" ]]; then
    local new_input
    new_input=$(extract_tool_input_from_modified_payload "$modified_payload" "$is_mcp" "$tool_input_json")
    if [[ "$new_input" != "$tool_input_json" ]]; then
      log_info "$LOGFILE" "Applying guardrail-modified tool_input for $tool_name"
    fi
    pretool_allow_output "$new_input" "Tool request allowed (Akto guardrails)"
    exit 0
  fi
