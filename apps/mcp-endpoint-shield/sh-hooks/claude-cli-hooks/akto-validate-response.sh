...
build_ingestion_payload() {
  ...
  local device_id host installer_hdrs now_ms
  ...
  jq -n -c \
    --arg mode "$MODE" --arg av "$AKTO_CONNECTOR_VALUE" --arg src "$CONTEXT_SOURCE" \
    ...
    ({"gen-ai":"Gen AI"} + (if $mode=="atlas" then {"ai-agent":$av,"source":$src} else {} end)) as $tags
    | ({"host":$host,"x-claude-hook":"Stop","content-type":"application/json"} + $inst) as $reqh
    | {
        path: "/v1/messages",
        requestHeaders: ($reqh|tojson),
        responseHeaders: ({"x-claude-hook":"Stop","content-type":"application/json"}|tojson),
        method: "POST",
...
call_guardrails() {
  ...
  if [[ "$LOG_PAYLOADS" == "true" ]]; then
    log_info "$LOGFILE" "Response: $response_text"
  else
...
ingest_blocked_response() {
  ...
  req_body=$(jq -c --arg reason "${reason:-Policy violation}" '
    .responseHeaders = ({"x-claude-hook":"Stop","x-blocked-by":"Akto Proxy","content-type":"application/json"} | tojson)
...
send_ingestion_data() {
  ...
  if [[ "$LOG_PAYLOADS" == "true" ]]; then
    log_info "$LOGFILE" "Prompt: $user_prompt"
    log_info "$LOGFILE" "Response: $response_text"
  else
