...
def validate_tool_use(
    ...
    *,
    ...
    if LOG_PAYLOADS:
        logger.info(f"Tool args (full): {tool_args[:2000]}")
    else:
...
def main():
    ...
    logger.info(
        ...
        f"cwd={cwd!r}, timestamp={timestamp}"
    )
    if LOG_PAYLOADS:
    ...
    if not allowed:
        ...
        # permissionDecision (top-level): honored by the Copilot CLI on exit 0.
        # hookSpecificOutput.permissionDecision: honored by VS Code (Claude-style).
        # systemMessage: surfaced to the user in VS Code. Covers both frontends.
        output = {
            "permissionDecision": "deny",
            "permissionDecisionReason": denial_reason,
            "systemMessage": denial_reason,
            "hookSpecificOutput": {
                "permissionDecision": "deny",
                "permissionDecisionReason": denial_reason,
            },
...
if __name__ == "__main__":
    try:
        ...
        logging.getLogger(__name__).exception("Pre-tool hook crashed with uncaught exception")
        sys.exit(0)
