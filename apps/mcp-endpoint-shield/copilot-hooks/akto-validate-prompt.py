...
def main():
    ...
    if AKTO_SYNC_MODE and AKTO_DATA_INGESTION_URL:
        ...
        if not allowed:
            ...
            sys.stderr.flush()
            # continue/stopReason: honored by VS Code (UserPromptSubmit output IS processed).
            # systemMessage: shown to the user in VS Code. The Copilot CLI ignores all of this
            # (userPromptSubmitted output is not processed), so on the CLI this is monitoring-only.
            output = {"continue": False, "stopReason": block_reason, "systemMessage": block_reason}
