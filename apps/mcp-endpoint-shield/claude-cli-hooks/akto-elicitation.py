#!/usr/bin/env python3
"""
Elicitation hook - validates MCP elicitation requests via guardrails.
...
def main():
    logger.info(f"=== Elicitation hook started - Mode: {MODE} ===")

    try:
        input_data = json.load(sys.stdin)
        logger.info("Elicitation input:\n%s", json.dumps(input_data, indent=2))
        ...
        if not allowed:
            ...
            logger.warning(f"BLOCKING Elicitation for {mcp_server}: {reason}")
            print(json.dumps({
                "hookSpecificOutput": {
