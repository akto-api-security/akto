...
def send_ingestion_data(tool_name: str, tool_input: Any, tool_response: Any):
    ...
    try:
        request_body = build_ingestion_payload(tool_name, tool_input, tool_response)
        post_payload_json(
...
def main():
    ...
    logger.info(f"Session: {session_id}, Processing tool response: {tool_name}")
    send_ingestion_data(tool_name, tool_input, tool_response)
