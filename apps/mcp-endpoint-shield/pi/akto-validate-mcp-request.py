...
def parse_mcp_tool(tool_name: str) -> Tuple[bool, str, str]:
    """
    Parse Pi tool_name into (is_mcp, server_name, mcp_tool_name).
...
def build_validation_request(
    ...
    *,
    ...
    response_headers = json.dumps({"x-pi-hook": "PreToolUse"})
    if is_mcp:
...
def call_guardrails(
    ...
    *,
    ...
    if is_mcp:
        logger.info(f"Validating MCP tools/call for {mcp_tool_name} (server={mcp_server_name}, piTool={tool_name})")
    else:
