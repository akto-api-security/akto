...
# Configure logging
LOG_DIR = os.path.expanduser(os.getenv("LOG_DIR", "~/.pi/akto/logs"))
...
AKTO_CONNECTOR = os.getenv("AKTO_CONNECTOR", "pi")
AKTO_CONNECTOR_VALUE = os.getenv("AKTO_CONNECTOR_VALUE", "pi")
...
if MODE == "atlas":
    PI_API_URL = f"https://{DEVICE_ID}.ai-agent.{AKTO_CONNECTOR_VALUE}" if DEVICE_ID else "https://api.pi.dev"
    logger.info(f"MODE: {MODE}, Device ID: {DEVICE_ID}, PI_API_URL: {PI_API_URL}")
else:
    PI_API_URL = os.getenv("PI_API_URL", "https://api.pi.dev")
    logger.info(f"MODE: {MODE}, PI_API_URL: {PI_API_URL}")
...
def parse_mcp_tool(tool_name: str) -> Tuple[bool, str, str]:
...
def build_ingestion_payload(
    ...
    *,
    ...
    if is_mcp:
        ...
        host = PI_API_URL.replace("https://", "").replace("http://", "")

    req_hdr: Dict[str, str] = {
        "host": host,
        "x-pi-hook": "PostToolUse",
    ...
    response_headers = json.dumps(
        {"x-pi-hook": "PostToolUse", "content-type": "application/json"}
    )
    if is_mcp:
...
def call_guardrails(
    ...
    *,
    ...
    if is_mcp:
        logger.info(
            f"Validating MCP tools/call result for {mcp_tool_name} (server={mcp_server_name}, piTool={tool_name})"
        )
    else:
...
def send_ingestion_data(
    ...
    *,
    ...
    if is_mcp:
        logger.info(
            f"Ingesting MCP tools/call result for {mcp_tool_name} (server={mcp_server_name}, piTool={tool_name})"
        )
    else:
...
def main():
    ...
    is_mcp, mcp_server_name, mcp_tool_name = parse_mcp_tool(tool_name)
