...
# Configuration
LOG_DIR = os.path.expanduser(os.getenv("LOG_DIR", "~/akto/.github/akto/vscode/logs"))
...
def build_akto_request(prompt: str, cwd: str, timestamp: int) -> Dict[str, Any]:
    ...
    request_headers = json.dumps({
        "host": host,
        "x-vscode-hook": "UserPromptSubmitted",
...
def ingest_allowed_request(prompt: str, cwd: str, timestamp: int):
    """Ingest allowed prompt to Akto for analytics."""
    if not AKTO_DATA_INGESTION_URL:
        return

    logger.info("Ingesting allowed prompt")
    try:
        request_body = build_akto_request(prompt, cwd, timestamp)
        ...
        logger.info("Allowed prompt ingested successfully")
    except Exception as e:
        logger.error(f"Ingestion error: {e}")


def ingest_request(prompt: str, cwd: str, timestamp: int, reason: str, blocked: bool = False):
...
def main():
    ...
    if AKTO_SYNC_MODE and AKTO_DATA_INGESTION_URL:
        ...
        if not allowed:
            ...
            ingest_request(prompt, cwd, timestamp, reason, blocked=True)
            # Warn user (but cannot block - GitHub limitation)
            sys.stderr.write(f"⚠️  Akto Guardrails flagged prompt: {reason}\n")
            sys.stderr.flush()
            sys.exit(2)
        else:
            ingest_request(prompt, cwd, timestamp, reason, blocked=False)
