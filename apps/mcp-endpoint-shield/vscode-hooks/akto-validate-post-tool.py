...
def ingest_tool_result(
    ...
    try:
        request_body = build_akto_request(tool_name, tool_args, tool_response, cwd, timestamp)
...
def main():
    ...
    if LOG_PAYLOADS:
        ...
        logger.debug(f"Result: {tool_response[:500]}")
    else:
        logger.info(f"Result preview: {tool_response[:100]}...")

    # Ingest the tool execution result (async mode - fire and forget)
    ingest_tool_result(tool_name, tool_args, tool_response, cwd, timestamp)
