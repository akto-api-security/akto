#!/usr/bin/env python3
"""
Akto OpenCode Tool Request Validation Hook
Validates tool calls against Akto guardrails before execution
...
file_handler = logging.FileHandler(os.path.join(LOG_DIR, "validate-tool-request.log"))
...
def build_validation_request(tool_name: str, tool_input: Any, tool_server_name: str) -> Dict[str, Any]:
    tags = {"gen-ai": "Gen AI", "tool_server_name": tool_server_name}
    if MODE == "atlas":
...
def call_guardrails(tool_name: str, tool_input: Any, tool_server_name: str) -> Tuple[bool, str]:
    if not tool_input:
        return True, ""
    ...
    logger.info(f"Validating tool request for tool: {tool_name} (server: {tool_server_name})")
    if LOG_PAYLOADS:
...
def ingest_blocked_request(tool_name: str, tool_input: Any, tool_server_name: str, reason: str):
...
def main():
    ...
    logger.info(f"Processing tool request: {tool_name} (server: {tool_server_name})")

    if AKTO_SYNC_MODE:
        allowed, reason = call_guardrails(tool_name, tool_input, tool_server_name)
        if not allowed:
            ...
            ingest_blocked_request(tool_name, tool_input, tool_server_name, block_reason)
            sys.exit(0)
