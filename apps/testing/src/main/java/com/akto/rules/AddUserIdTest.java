...
public class AddUserIdTest extends TestPlugin {
    ...
    public Result start(ApiInfo.ApiInfoKey apiInfoKey, AuthMechanism authMechanism, Map<ApiInfo.ApiInfoKey, List<String>> sampleMessages, Map<String, SingleTypeInfo> singleTypeInfos) {
        ...
        List<String> userIdNameList = Arrays.asList("user", "userId", "user_id", "customer_id", "customerId", "customer");
        List<SingleTypeInfo> validUserIdNameList = new ArrayList<>();
        for (SingleTypeInfo singleTypeInfo: singleTypeInfos.values()) {
            String param = singleTypeInfo.getParam();
            if (singleTypeInfo.getValues().getElements().isEmpty()) continue; // todo:
            if (userIdNameList.contains(param)) validUserIdNameList.add(singleTypeInfo);
        }

        RawApi rawApi = filteredMessages.get(0);
        List<TestResult> testResults = new ArrayList<>();
        boolean overallVulnerable = false;
        for (SingleTypeInfo singleTypeInfo: validUserIdNameList) {
            OriginalHttpRequest testRequest = rawApi.getRequest().copy();
            String key = singleTypeInfo.getParam();
            String val = (String) singleTypeInfo.getValues().getElements().toArray()[0]; // todo:
            ...
            boolean vulnerable = isStatusGood(apiExecutionDetails.statusCode) && apiExecutionDetails.percentageMatch < 90;
            overallVulnerable = vulnerable;
