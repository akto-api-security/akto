...
_SIGNALS: List[Tuple[str, "re.Pattern", float]] = [
    ...
    ("shell", re.compile(
        r"(?:#!/|\bos\.system\b|\bsubprocess\b|\brm\s+-rf\b|\bsudo\b|\bchmod\b|\bapt-get\b)"), 0.5),
    ("sql", re.compile(
        r"\b(?:SELECT\s+.+\s+FROM|INSERT\s+INTO|UPDATE\s+\w+\s+SET|DELETE\s+FROM|"
        r"CREATE\s+TABLE|DROP\s+TABLE)\b", re.IGNORECASE), 0.5),
