...
#
# The semantic cache (embedder + Redis) now lives in guardrails-service and is
# deployed with it, not here.
...
echo "Deploying ${WORKER_IMAGE}:${IMAGE_TAG} + ${ANONYMIZER_IMAGE}:${IMAGE_TAG} to RG ${RG}"
