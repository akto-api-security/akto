...
Models on Azure AI Foundry instead of Vertex: secrets `QWEN3GUARD_FOUNDRY_API_KEY` / `GEMMA_FOUNDRY_API_KEY` / `AZURE_FOUNDRY_API_KEY` / `ANTHROPIC_FOUNDRY_API_KEY`; plain env `*_FOUNDRY_BASE_URL` (endpoint URL from the Foundry portal) and optional `*_FOUNDRY_DEPLOYMENT` / `*_FOUNDRY_MODEL`, with a `DEFAULT_MODEL_CONFIG_JSON` referencing the `qwen3guard_foundry` / `gemma_foundry` providers (see [.env.example](../.env.example)). Claude on Foundry uses the native Anthropic Messages route via the `anthropic_foundry` provider (`ANTHROPIC_FOUNDRY_BASE_URL` = the `.../anthropic` endpoint) — not the OpenAI-compatible `azure_foundry` provider.
