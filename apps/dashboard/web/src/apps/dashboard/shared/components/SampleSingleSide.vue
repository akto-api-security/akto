<template>
  <div class="sample-data-container">
      ...
      <div class="copy-icon">
        <simple-menu v-if="!simpleCopy" :items="copyMenuItems">
          <template v-slot:activator2>
            <v-icon size=16 class="tray-button">
              $fas_copy
            </v-icon>
          </template>
        </simple-menu>
        <v-tooltip bottom v-else>
          <template v-slot:activator='{ on, attrs }'>
            <v-icon size=16 class="tray-button" v-bind="attrs" v-on="on" @click="copyRequest">
              $fas_copy
            </v-icon>
          </template>
    ...
    <div class="wrapper">
      <json-viewer :contentJSON="data['json']" :errors="{}" :highlightItemMap="data['highlightPaths']" />
    </div>
  </div>
</template>
