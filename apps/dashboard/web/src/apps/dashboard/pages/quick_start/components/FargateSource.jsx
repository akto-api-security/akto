...
import { Button, HorizontalStack, ProgressBar, Text, VerticalStack } from '@shopify/polaris'
...
function FargateSource() {
    ...
    const copyYaml = () => {
      ...
      setToast(true, false, "Variables copied !!")
    }
    ...
    const accessComponent = (
      ...
      :
        <VerticalStack gap="2">
          {isButtonActive ? <Button primary onClick={creatFargateStack} loading={loading}>Setup traffic processors</Button> : null}
          ...
          {stackStatus === "CREATE_COMPLETE" ?
              <VerticalStack gap="2">
                ...
                <JsonComponent title="Variables" toolTipContent="Copy your variables" onClickFunc={()=> copyYaml()} contentValue={yamlContent} language="yaml"/>
              </VerticalStack>
              : null
          }
        </VerticalStack>
    ...
    const localDeployObj = {
        text:  "Use generic traffic collector to send traffic to Akto.",
        component: <BannerComponent title="Setup using Fargate" docsUrl={docsUrl} content="Akto container config can duplicate your container-traffic and send to Akto dashboard." />
    }

    const noAccessObject = {
      text: "Use this for AWS Fargate, AWS ECS, TCP-collector, Docker, Docker-compose. Your dashboard's instance needs relevant access to setup traffic processors, please do the following steps:",
