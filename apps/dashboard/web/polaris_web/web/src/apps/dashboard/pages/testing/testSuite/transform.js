...
const transform  = {
    hasTestsInSuite: (testSuiteItem) => (testSuiteItem?.subCategoryList?.length ?? 0) > 0,
    shouldShowDefaultTestSuiteOnManagementPage: (suiteType) => {
        if (!suiteType) {
            return false;
        }
        if (isMCPSecurityCategory()) {
            return suiteType === 'MCP_SECURITY';
        }
        if (isAgenticSecurityCategory()) {
            return suiteType === 'MCP_SECURITY'
                || suiteType === 'AI_AGENT_SECURITY'
                || (suiteType.includes('ATTACK'));
        }
        return suiteType === 'OWASP';
    },
