...
function Splunk() {
    ...
    const [postmanKey, setSplunkUrl] = useState('');
    const [workspaces, setSplunkToken] = useState([]);
    ...
    async function fetchSplunkCred() {
    ...
    const PostmanCard = (
        ...
        >
          ...
          <LegacyCard.Section>
            <PasswordTextField text={splunkKey} helpText="Paste your Splunk api token here."
                                setField={setSplunkKey} onFunc={true} field={splunkKey} 
                                label="Splunk API token"
            />
            <br/>
            <TextField text={splunkKey} helpText="Add the Url of splunk Http Collector"
                                setField={setSplunkUrl} onFunc={true} field={splunk} 
                                label="Splunk API token"
            />
            <Dropdown 
              helpText="Add Splunk Http Event Collector Url" 
              menuItems={workspaces} selected={handleSelectChange} 
              initial={selected}
              label="Splunk Http Url"/>
          </LegacyCard.Section> 
