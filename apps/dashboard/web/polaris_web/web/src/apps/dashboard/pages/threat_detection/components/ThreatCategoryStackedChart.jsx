...
function ThreatCategoryStackedChart({ startTimestamp, endTimestamp }) {
  ...
  useEffect(() => {
    ...
    const loadThreatData = async () => {
      ...
      try {
        ...
        const resp = await api.getThreatActivityTimeline(startTimestamp, endTimestamp)
        ...
        if (!resp) {
          ...
          raw = resp
        } else if (Array.isArray(resp.threatActivityTimelines)) {
          raw = resp.threatActivityTimelines
        } else if (Array.isArray(resp.threatActivityTimeline)) {
          raw = resp.threatActivityTimeline
        } else if (Array.isArray(resp.threat_activity_timeline)) {
          raw = resp.threat_activity_timeline
        } else if (resp.data) {
          const d = resp.data
          if (Array.isArray(d.threatActivityTimelines)) raw = d.threatActivityTimelines
          else if (Array.isArray(d.threatActivityTimeline)) raw = d.threatActivityTimeline
          else if (Array.isArray(d.threat_activity_timeline)) raw = d.threat_activity_timeline
        ...
        const categories = dayKeys.map(k => dayjs(k * 1000).format('ddd, D MMM'))
        ...
        const TOP_N = 6
        const sortedSubs = Array.from(subSet).sort((a, b) => (totals[b] || 0) - (totals[a] || 0))
        ...
        dayKeys.forEach((k, di) => {
          Object.entries(dayMap.get(k)).forEach(([sk, val]) => {
            ...
            if (topSubs.includes(sk)) {
              ...
              seriesMap['Other'][di] = Number(seriesMap['Other'][di] || 0) + numericVal
            }
          })
        })
        ...
        const initialLatest = series.map((s) => ({
          name: s.name,
          rawName: s.rawName,
          percent: grandSum > 0 ? Math.round(((grandTotals[s.rawName] || 0) / grandSum) * 1000) / 10 : 0,
          color: s.color
        ...
        const opts = {
          ...
          yAxis: {
            title: { text: '' },
            labels: { formatter() { return this.value + '%' }, style: { color: '#6B7280', fontSize: '11px' } },
            max: 100,
          ...
          tooltip: {
            ...
            formatter() {
              ...
              this.points.forEach(p => {
                const percentage = total > 0 ? ((p.y / total) * 100).toFixed(1) : 0
                tooltipHtml +=
          ...
          plotOptions: { area: { stacking: 'percent', marker: { enabled: false }, lineWidth: 0, states: { hover: { lineWidthPlus: 0 } } } },
          legend: { enabled: false },
          series
        }

  // keep base options and totals for toggling updates
  baseOptionsRef.current = { ...opts, _totals: totals, _seriesKeys: seriesKeys }
  setOptions(opts)
      } catch (err) {
        if (mounted) {
          setOptions(null)
  ...
  useEffect(() => {
    ...
    const newSeries = (base.series || []).map(s => ({ ...s, visible: visibleSeries[s.name] !== false }))

    setLatestPercents(prev => prev.map(item => ({ ...item, visible: visibleSeries[item.name] !== false })))
                  ...
                  {latestPercents.map(item => (
                    ...
                    >
                      ...
                      <div style={{ fontSize: 13, color: '#374151' }}>
                        <span style={{ fontWeight: 600, color: '#111827' }}>{item.percent}%</span>
                        {' '}
              ...
              <div style={{ width: '100%', padding: '8px 0' }}>
                {options ? (
                  ...
                  <div style={{
                    ...
                    No threat activity data found for the selected time period. Please check the console for debugging information.
                  </div>
                )}
              </div>
            </div>
