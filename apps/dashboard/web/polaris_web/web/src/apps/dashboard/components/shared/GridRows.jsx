...
function GridRows(props) {
    ...
    const {disable, columns , items, CardComponent, buttonText, onButtonClick, changedColumns} = props;
                ...
                {Array.from({length: rows}).map((_,index)=>(
                    <HorizontalGrid columns={{xs: 1, sm: Math.max(usedColumns - 2,1) , md: usedColumns - 1 , lg:usedColumns, xl: usedColumns}} gap="5" key={(index + 1) * 1000}>
                        {Array.from({ length: usedColumns }).map((_, col) => {
                            ...
                            if (item) {
                                return <CardComponent disable={disable} cardObj={item} buttonText={buttonText} key={itemIndex} onButtonClick={onButtonClick}/>;
                            }
                        })}
                    </HorizontalGrid>
                ))}
