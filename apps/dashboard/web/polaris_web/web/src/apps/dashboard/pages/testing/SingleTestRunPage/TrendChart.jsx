...
function TrendChart(props) {
        ...
        ret.push({
          ...
          filter: (
            <ChoiceList
              ...
              selected={
                appliedFilters.filter((localFilter) => {
                  return localFilter.key == key;
                }).length == 1
                  ? appliedFilters.filter((localFilter) => {
                      return localFilter.key == key;
                    })[0].value
  ...
  async function fetchData(dateRange, firstTime) {
    ...
    await api
      .fetchTestingRunResultSummaries(hexId, st, en)
      .then(async ({ testingRunResultSummaries }) => {
        ...
        testingRunResultSummaries.forEach((ele) => {
          let obj =
            ele?.countIssues && Object.keys(ele.countIssues).length > 0
              ? ele.countIssues
        ...
        if (firstTime) {
          const result = testingRunResultSummaries.reduce(
            ...
            {
              minEndTimestamp: values.todayDayEnd / 1000,
              maxStartTimestamp:
                new Date().setDate(values.today.getDate() - 6) / 1000,
            }
          );
          ...
          dispatchCurrDateRange({
            type: "update",
            period: { period: obj, title: "Custom", alias: "custom" },
          });
        }
      });
  }
  ...
  function tooltipFormatter(tooltip) {
    ...
    if (tooltipMetadata?.[this.x]) {
      ...
      if (tooltipMetadata[this.x].repository) {
        def.push(
          `<span>repository: ${tooltipMetadata[this.x].repository}</span><br/>`
        );
      }
    }
    return def;
  ...
  return (
    <LegacyCard>
      <LegacyCard.Section
        title={
          <Text fontWeight="regular" variant="bodySm" color="subdued">
            Vulnerabilities
          </Text>
        }
      >
        <HorizontalStack align="space-between">
          <Text fontWeight="semibold" variant="bodyMd">
            Found {props?.totalVulnerabilities || totalVulnerabilities}{" "}
