...
export default function AgenticTopListCard({ title, columns = [], rows = [], emptyStateText = "No data available", renderIcon, activeRows }) {
    ...
    const tableRows = rows.length > 0
        ? rows.map((row) => {
            ...
            return [
                <div key={`name-${row.id}`} className="agentic-list-cell-click" data-active={isActive} onClick={() => row.onClick?.(row)}>
                ...
                <div key={`val-${row.id}`} className="agentic-list-cell-click" data-active={isActive} onClick={() => row.onClick?.(row)}>
                    {row.renderValue(row)}
                </div>,
            ];
        })
