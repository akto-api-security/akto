...
const tableFunc = {
    ...
    fetchDataSync: function (sortKey, sortOrder, skip, limit, filters, filterOperators, queryValue, setFilters, props){
        ...
        let localFilters = func.prepareFilters(tempData, props.tableFilterConfig || props.filters);
        ...
        (props.tableFilterConfig || props.filters || []).forEach((filter) => {
          if (filter?.key) validHeaderKeys.add(filter.key);
        });
