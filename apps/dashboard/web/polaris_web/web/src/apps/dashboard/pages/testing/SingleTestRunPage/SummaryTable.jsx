...
function SummaryTable({ testingRunResultSummaries, setSummary }) {
  ...
  const promotedBulkActions = (selectedTestRuns) => {
    return [
      {
        content: `Delete ${selectedTestRuns.length} test summar${
          selectedTestRuns.length > 1 ? "ies" : "y"
        }`,
        onAction: async () => {
          ...
          func.setToast(
            true,
            false,
            `${selectedTestRuns.length} test summar${
              selectedTestRuns.length > 1 ? "ies" : "y"
  ...
  useEffect(() => {
    if (testingRunResultSummaries && testingRunResultSummaries.length > 0) {
      const prettifiedData = transform.prettifySummaryTable(
  ...
  let tmp =
    data && data.length <= pageLimit
      ? data
      : data.slice(
          page * pageLimit,
          Math.min((page + 1) * pageLimit, data.length)
        );
  let rowMarkup = tmp.map((data, index) => (
    <GithubRow
      key={data.id}
      id={data.id}
      dataObj={data}
              ...
              label={
                total === 0 ? (
                  ...
                  <div data-testid="pagination-label">
                    {`Showing ${observeFunc.formatNumberWithCommas(
                      page * pageLimit + Math.min(1, total)
                    )}-${observeFunc.formatNumberWithCommas(
                      Math.min((page + 1) * pageLimit, total)
                    )} of ${observeFunc.formatNumberWithCommas(total)}`}
                  </div>
                )
              }
              hasPrevious={page > 0}
