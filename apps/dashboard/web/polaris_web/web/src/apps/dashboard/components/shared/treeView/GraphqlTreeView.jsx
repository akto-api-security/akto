...
import { buildGraphQLTree } from './graphqlTransform'
import GraphqlTreeTable from './GraphqlTreeTable'

// Columns irrelevant in tree context
const SKIP_COLUMNS = new Set([
    'methodComp', 'sourceLocationComp',
    'descriptionComp', 'lastTestedComp', 'componentRiskAnalysisComp'
])
...
function GraphqlTreeView({ endpoints, prettifyEndpoints, onTerminalClick, tableHeaders }) {
    ...
    const endpointHeaders = useMemo(() => {
        if (!tableHeaders) return []
        return tableHeaders
            .filter(h => h.value && !SKIP_COLUMNS.has(h.value))
            .map(h => ({
                ...h,
                title: typeof h.title === 'string' ? h.title : (h.text || h.value || ''),
            }))
    }, [tableHeaders])

    useEffect(() => {
        const readyEndpoints = prettifyEndpoints
            ? prettifyEndpoints(endpoints || [])
            : (endpoints || [])

        const tree = buildGraphQLTree(readyEndpoints, groupByOperation)
        setTreeData(tree)
    }, [endpoints, prettifyEndpoints, groupByOperation])
