...
public class InventoryAction extends UserAction {
    ...
    private ApiInfo toApiInfo(EndpointInfoView view) {
        ...
        // the view stores auth types as plain strings (master turned ApiInfo.AuthType into string
        // constants); on this branch it is still an enum, so parse rather than assign. an unknown
        // value is dropped instead of failing the row. never leave this null — the table calls
        // actualAuthType.join(", ") unguarded and a null takes out the whole page.
        List<ApiInfo.AuthType> actualAuthType = new ArrayList<ApiInfo.AuthType>();
        if (view.getActualAuthType() != null) {
            for (String authType: view.getActualAuthType()) {
                try {
                    actualAuthType.add(ApiInfo.AuthType.valueOf(authType));
                } catch (Exception ignored) {
                }
            }
        }
        apiInfo.setActualAuthType(actualAuthType);
