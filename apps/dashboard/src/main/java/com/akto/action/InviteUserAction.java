...
public class InviteUserAction extends UserAction{
     ...
     * @param userRole the current user's role (for hierarchy checking)
    ...
    public String execute() {
        ...
        if (this.scopeRoleMapping != null && !this.scopeRoleMapping.isEmpty()) {
            ...
            if (scopeRoleToSave.isEmpty()) {
                // Default to API + default if no scopes selected
                loggerMaker.debugAndAddToDb("scopeRoleMapping is empty, defaulting to API + defaultrole");
                ...
                scopeRoleToSave.put("API", defaultRole);
            }
