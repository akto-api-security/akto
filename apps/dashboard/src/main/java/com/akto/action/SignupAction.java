    ...
    public String registerViaAuth0() throws Exception {
        ...
        if(parsedState.containsKey("signupInvitationCode")){
            ...
            if (pendingInviteCode != null && pendingInviteCode.getInviteeEmailId().equals(email)) {
                ...
                // Default to API + MEMBER if scopeRoleMapping is empty or null
...
//    }
    ...
    private void createUserAndRedirect(String userEmail, String username, SignupInfo signupInfo,
        ...
        if (user == null && "false".equalsIgnoreCase(shouldLogin)) {
            ...
            if (user == null) {
                ...
                if(invitedRole != null && accountId != 0){
                    ...
                    if(!exists){
                        ...
                        }else {   //if only role is present while creating user then map that role to API as this is the case of new sign up without invitation
                            this.scopeRoleMapping.put("API", invitedRole);
                            rbacEntry.setScopeRoleMapping(this.scopeRoleMapping);
                        }
            ...
            // Set scope-role mapping for newly initialized user if available from invitation
            if (this.scopeRoleMapping != null && !this.scopeRoleMapping.isEmpty()) {
                try {
                    ...
                    logger.info("[createUserAndRedirect] Set scope-role mapping for new user: " + this.scopeRoleMapping);
                } catch (Exception e) {
                    logger.errorAndAddToDb(e, "[createUserAndRedirect] Error setting scope-role mapping for new user: " + e.getMessage());
                }
            }
