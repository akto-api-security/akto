...
public class UngovernedAiRatioProvider extends AbstractInsightProvider {
    ...
    public InsightResult compute(InsightDataBundle bundle, InsightContext ctx, Scope scope) {
        ...
        Map<GovernanceBucket, Integer> byBucket = new HashMap<>();
        ...
        for (ApiCollection c : bundle.collections) {
            ...
            total++;
            GovernanceBucket bucket = InsightUtil.governanceBucket(c, allowlistLower, remarksByService);
            byBucket.merge(bucket, 1, Integer::sum);

            if (bucket != GovernanceBucket.SANCTIONED) {
        ...
        int sanctioned = byBucket.getOrDefault(GovernanceBucket.SANCTIONED, 0);
        ...
        for (GovernanceBucket bucket : new GovernanceBucket[]{GovernanceBucket.SANCTIONED, GovernanceBucket.UNAPPROVED,
                GovernanceBucket.SHADOW, GovernanceBucket.LOCAL, GovernanceBucket.PERSONAL, GovernanceBucket.REJECTED,
                GovernanceBucket.MALICIOUS}) {
