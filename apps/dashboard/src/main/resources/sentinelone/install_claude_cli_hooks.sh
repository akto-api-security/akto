#!/bin/bash

# ========================================================================================
# MCP Endpoint Shield - Claude CLI Hook Installer
# ========================================================================================
# Automatically installs Akto guardrails hooks for Claude CLI if detected
# Downloads latest hooks from GitHub and configures Claude CLI settings
# Compatible with macOS and Linux
# Mirrors: mcp-endpoint-shield/misc/macos/install_claude_hooks.sh (master branch, full hook set)
# ========================================================================================

# Ensure common binary paths are available (RTR/remote execution uses minimal PATH)
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:$PATH"

set -e

GITHUB_RAW_BASE="https://raw.githubusercontent.com/akto-api-security/akto/master/apps/mcp-endpoint-shield/claude-cli-hooks"
GITHUB_SHARED_BASE="https://raw.githubusercontent.com/akto-api-security/akto/master/apps/mcp-endpoint-shield/shared"

# Detect OS
OS_TYPE="$(uname -s)"
IS_MACOS=false
IS_LINUX=false

case "$OS_TYPE" in
    Darwin) IS_MACOS=true ;;
    Linux) IS_LINUX=true ;;
esac

TARGET_USER_HOME="${TARGET_USER_HOME:-}"
AKTO_DATA_INGESTION_URL="${AKTO_DATA_INGESTION_URL:-}"
AKTO_API_TOKEN="${AKTO_API_TOKEN:-}"
for a in "$@"; do
    case "$a" in
        TARGET_USER_HOME=*) TARGET_USER_HOME="${a#TARGET_USER_HOME=}" ;;
        AKTO_DATA_INGESTION_URL=*) AKTO_DATA_INGESTION_URL="${a#AKTO_DATA_INGESTION_URL=}" ;;
        AKTO_API_TOKEN=*) AKTO_API_TOKEN="${a#AKTO_API_TOKEN=}" ;;
    esac
done

[ -n "$AKTO_DATA_INGESTION_URL" ] && export AKTO_DATA_INGESTION_URL
[ -n "$AKTO_API_TOKEN" ] && export AKTO_API_TOKEN

log() {
    echo "[Claude CLI Hooks] $1"
}

log_error() {
    echo "[Claude CLI Hooks] ERROR: $1" >&2
}

install_for_user() {
    local user_home="$1"

    export HOME="$user_home"
    TARGET_USER_HOME="$user_home"
    CLAUDE_HOOKS_DIR="$user_home/.claude/hooks"
    CLAUDE_SETTINGS_FILE="$user_home/.claude/settings.json"

    if main; then
        return 0
    else
        log_error "Installation failed for $user_home"
        return 1
    fi
}

check_claude_installed() {
    # Check Claude config directory (most reliable — exists after first run)
    if [ -d "$TARGET_USER_HOME/.claude" ]; then
        return 0
    fi

    # Check common install paths (npm global, standalone installer)
    if [ -f "/usr/local/bin/claude" ] || [ -f "/usr/bin/claude" ] || \
       [ -f "$TARGET_USER_HOME/.local/bin/claude" ] || \
       [ -f "$TARGET_USER_HOME/.npm-global/bin/claude" ]; then
        return 0
    fi

    # command -v may fail under sudo (different PATH), try it last
    if command -v claude >/dev/null 2>&1; then
        return 0
    fi

    return 1
}

get_ingestion_url() {
    if [ -n "${AKTO_DATA_INGESTION_URL:-}" ]; then
        echo "$AKTO_DATA_INGESTION_URL"
        return 0
    fi
    echo "https://guardrails.akto.io"
}

get_api_token() {
    echo "${AKTO_API_TOKEN:-}"
}

generate_device_id() {
    # macOS: Try ioreg first
    if [ "$IS_MACOS" = true ] && command -v ioreg >/dev/null 2>&1; then
        UUID=$(ioreg -rd1 -c IOPlatformExpertDevice 2>/dev/null | grep IOPlatformUUID | awk -F'"' '{print $4}')
        if [ -n "$UUID" ]; then
            echo "$UUID" | tr -d '-' | tr '[:upper:]' '[:lower:]'
            return 0
        fi
    fi

    # Linux: Try machine-id
    if [ "$IS_LINUX" = true ] && [ -f "/etc/machine-id" ]; then
        cat "/etc/machine-id" | tr '[:upper:]' '[:lower:]'
        return 0
    fi

    if [ "$IS_LINUX" = true ] && [ -f "/var/lib/dbus/machine-id" ]; then
        cat "/var/lib/dbus/machine-id" | tr '[:upper:]' '[:lower:]'
        return 0
    fi

    # Fallback: Get MAC address (macOS: en0, Linux: eth0 or ens0)
    if command -v ifconfig >/dev/null 2>&1; then
        if [ "$IS_MACOS" = true ]; then
            MAC=$(ifconfig en0 2>/dev/null | grep ether | awk '{print $2}' | tr -d ':')
        else
            MAC=$(ifconfig eth0 2>/dev/null | awk '/ether/{print $2}' | tr -d ':')
            [ -z "$MAC" ] && MAC=$(ifconfig ens0 2>/dev/null | awk '/ether/{print $2}' | tr -d ':')
        fi
        if [ -n "$MAC" ]; then
            echo "$MAC" | tr '[:upper:]' '[:lower:]'
            return 0
        fi
    fi

    # Last resort: ip command (Linux only) — no grep -oP, use awk instead
    if [ "$IS_LINUX" = true ] && command -v ip >/dev/null 2>&1; then
        MAC=$(ip link show | awk '/link\/ether/{print $2}' | head -1 | tr -d ':')
        if [ -n "$MAC" ]; then
            echo "$MAC" | tr '[:upper:]' '[:lower:]'
            return 0
        fi
    fi

    echo ""
}

download_file() {
    local url="$1"
    local dest="$2"

    if command -v curl >/dev/null 2>&1; then
        curl -fsSL -H "Cache-Control: no-cache" -H "Pragma: no-cache" "$url" -o "$dest"
        return $?
    elif command -v wget >/dev/null 2>&1; then
        wget -q --no-cache "$url" -O "$dest"
        return $?
    else
        log_error "Neither curl nor wget found. Cannot download hooks."
        return 1
    fi
}

# Download and configure wrapper script for prompt hook
create_prompt_wrapper() {
    local hook_type="$1"  # "prompt" or "response"
    local ingestion_url="$2"
    local device_id="$3"

    local wrapper_file="$CLAUDE_HOOKS_DIR/akto-validate-${hook_type}-wrapper.sh"
    local template_url="$GITHUB_RAW_BASE/akto-validate-${hook_type}-wrapper.sh"
    local python_script="$CLAUDE_HOOKS_DIR/akto-validate-${hook_type}.py"

    if ! download_file "$template_url" "$wrapper_file.tmp"; then
        log_error "Failed to download wrapper template for $hook_type, creating locally..."
        log "Creating wrapper with URL: $ingestion_url"

        cat > "$wrapper_file" <<EOF
#!/bin/bash
# Auto-generated wrapper for Akto guardrails hook
# Data Ingestion URL: $ingestion_url

export MODE="atlas"
export AKTO_DATA_INGESTION_URL="$ingestion_url"
export AKTO_API_TOKEN="$API_TOKEN"
export AKTO_SYNC_MODE="true"
export AKTO_TIMEOUT="5"
export AKTO_CONNECTOR="claude_code_cli"
export CONTEXT_SOURCE="ENDPOINT"
export DEVICE_ID="$device_id"

echo "[Claude Hook] Data Ingestion URL: \$AKTO_DATA_INGESTION_URL" >&2
echo "[Claude Hook] Device ID: \$DEVICE_ID" >&2

exec python3 "$python_script" "\$@"
EOF
        chmod +x "$wrapper_file"
        return 0
    fi

    sed -e "s|{{AKTO_DATA_INGESTION_URL}}|$ingestion_url|g" \
        -e "s|{{AKTO_API_TOKEN}}|$API_TOKEN|g" \
        -e "s|{{DEVICE_ID (optional)}}|$device_id|g" \
        "$wrapper_file.tmp" > "$wrapper_file"

    rm -f "$wrapper_file.tmp"
    chmod +x "$wrapper_file"

    log "Wrapper configured with URL: $ingestion_url"
}

# Download and configure wrapper script for MCP hook
create_mcp_wrapper() {
    local hook_type="$1"  # "mcp-request" or "mcp-response"
    local ingestion_url="$2"
    local device_id="$3"

    local wrapper_file="$CLAUDE_HOOKS_DIR/akto-validate-${hook_type}-wrapper.sh"
    local template_url="$GITHUB_RAW_BASE/akto-validate-${hook_type}-wrapper.sh"

    if ! download_file "$template_url" "$wrapper_file.tmp"; then
        log_error "Failed to download MCP wrapper template for $hook_type"
        return 1
    fi

    sed -e "s|{{AKTO_DATA_INGESTION_URL}}|$ingestion_url|g" \
        -e "s|{{AKTO_API_TOKEN}}|$API_TOKEN|g" \
        -e "s|{{DEVICE_ID (optional)}}|$device_id|g" \
        "$wrapper_file.tmp" > "$wrapper_file"

    rm -f "$wrapper_file.tmp"
    chmod +x "$wrapper_file"
}

# Download akto-hook-wrapper.sh and substitute placeholders (drives observability hooks)
create_hook_wrapper() {
    local ingestion_url="$1"
    local device_id="$2"
    local wrapper_file="$CLAUDE_HOOKS_DIR/akto-hook-wrapper.sh"

    if ! download_file "$GITHUB_RAW_BASE/akto-hook-wrapper.sh" "$wrapper_file.tmp"; then
        log_error "Failed to download akto-hook-wrapper.sh, creating locally"
        cat > "$wrapper_file" <<EOF
#!/bin/bash
# Auto-generated wrapper for Akto observability hooks

export MODE="atlas"
export AKTO_DATA_INGESTION_URL="$ingestion_url"
export AKTO_API_TOKEN="$API_TOKEN"
export AKTO_SYNC_MODE="true"
export AKTO_TIMEOUT="5"
export AKTO_CONNECTOR="claude"
export CONTEXT_SOURCE="ENDPOINT"
export DEVICE_ID="$device_id"

SCRIPT_DIR="$CLAUDE_HOOKS_DIR"
exec python3 "\$SCRIPT_DIR/\$1" "\${@:2}"
EOF
        chmod +x "$wrapper_file"
        log "Created akto-hook-wrapper.sh"
        return 0
    fi

    sed -e "s|{{AKTO_DATA_INGESTION_URL}}|$ingestion_url|g" \
        -e "s|{{AKTO_API_TOKEN}}|$API_TOKEN|g" \
        -e "s|{{DEVICE_ID (optional)}}|$device_id|g" \
        "$wrapper_file.tmp" > "$wrapper_file"

    rm -f "$wrapper_file.tmp"
    chmod +x "$wrapper_file"
    log "Created akto-hook-wrapper.sh"
}

# Update or create Claude settings.json with prompt hooks (UserPromptSubmit / Stop)
update_claude_settings() {
    local prompt_wrapper="$CLAUDE_HOOKS_DIR/akto-validate-prompt-wrapper.sh"
    local response_wrapper="$CLAUDE_HOOKS_DIR/akto-validate-response-wrapper.sh"

    local new_hooks_config
    new_hooks_config=$(cat <<EOF
{
    "hooks": {
        "UserPromptSubmit": [
            { "hooks": [ { "type": "command", "command": "$prompt_wrapper", "timeout": 10 } ] }
        ],
        "Stop": [
            { "hooks": [ { "type": "command", "command": "$response_wrapper", "timeout": 10 } ] }
        ]
    }
}
EOF
)

    if [ -f "$CLAUDE_SETTINGS_FILE" ]; then
        log "Existing settings.json found, backing up..."
        cp "$CLAUDE_SETTINGS_FILE" "$CLAUDE_SETTINGS_FILE.backup"

        if command -v jq >/dev/null 2>&1; then
            jq --argjson newhooks "$new_hooks_config" '
                .hooks.UserPromptSubmit = $newhooks.hooks.UserPromptSubmit |
                .hooks.Stop = $newhooks.hooks.Stop
            ' "$CLAUDE_SETTINGS_FILE" > "$CLAUDE_SETTINGS_FILE.tmp"
            mv "$CLAUDE_SETTINGS_FILE.tmp" "$CLAUDE_SETTINGS_FILE"
            log "Merged prompt hooks into existing settings.json"
        else
            echo "$new_hooks_config" > "$CLAUDE_SETTINGS_FILE"
            log "Created new settings.json (no jq available for merge)"
        fi
    else
        echo "$new_hooks_config" > "$CLAUDE_SETTINGS_FILE"
        log "Created new settings.json with prompt hooks"
    fi
}

# Download MCP hook scripts and merge PreToolUse/PostToolUse into settings.json
install_mcp_hooks() {
    local ingestion_url="$1"
    local device_id="$2"

    log "Installing MCP hooks (PreToolUse/PostToolUse)..."

    for script in akto-validate-mcp-request.py akto-validate-mcp-response.py; do
        if ! download_file "$GITHUB_RAW_BASE/$script" "$CLAUDE_HOOKS_DIR/$script"; then
            log_error "Failed to download $script"
            return 1
        fi
        chmod +x "$CLAUDE_HOOKS_DIR/$script"
        log "Downloaded $script"
    done

    create_mcp_wrapper "mcp-request" "$ingestion_url" "$device_id"
    log "Created akto-validate-mcp-request-wrapper.sh"

    create_mcp_wrapper "mcp-response" "$ingestion_url" "$device_id"
    log "Created akto-validate-mcp-response-wrapper.sh"

    local tmp_settings="/tmp/akto-mcp-claude-settings-$$.json"
    if ! download_file "$GITHUB_RAW_BASE/settings.json" "$tmp_settings"; then
        log_error "Failed to download MCP settings.json"
        return 1
    fi

    if [ -f "$CLAUDE_SETTINGS_FILE" ]; then
        if command -v jq >/dev/null 2>&1; then
            jq --argjson mcp "$(cat "$tmp_settings")" '
                .hooks.PreToolUse = $mcp.hooks.PreToolUse |
                .hooks.PostToolUse = $mcp.hooks.PostToolUse
            ' "$CLAUDE_SETTINGS_FILE" > "$CLAUDE_SETTINGS_FILE.tmp"
            mv "$CLAUDE_SETTINGS_FILE.tmp" "$CLAUDE_SETTINGS_FILE"
            log "Merged MCP hooks (PreToolUse/PostToolUse) into settings.json"
        else
            cp "$tmp_settings" "$CLAUDE_SETTINGS_FILE"
            log "Created settings.json with MCP hooks (no jq available for merge)"
        fi
    else
        cp "$tmp_settings" "$CLAUDE_SETTINGS_FILE"
        log "Created settings.json with MCP hooks"
    fi

    rm -f "$tmp_settings"
}

# Merge observability hooks into settings.json (always runs)
install_observability_hooks() {
    local hooks_dir="$CLAUDE_HOOKS_DIR"
    local wrapper="bash $hooks_dir/akto-hook-wrapper.sh"

    local obs_hooks_config
    obs_hooks_config=$(cat <<EOF
{
  "hooks": {
    "SessionStart":       [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py SessionStart", "timeout": 10 } ] } ],
    "SessionEnd":         [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py SessionEnd", "timeout": 10 } ] } ],
    "InstructionsLoaded": [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py InstructionsLoaded", "timeout": 10 } ] } ],
    "PermissionRequest":  [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py PermissionRequest", "timeout": 10 } ] } ],
    "PermissionDenied":   [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py PermissionDenied", "timeout": 10 } ] } ],
    "PostToolUseFailure": [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py PostToolUseFailure", "timeout": 10 } ] } ],
    "StopFailure":        [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py StopFailure", "timeout": 10 } ] } ],
    "SubagentStart":      [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py SubagentStart", "timeout": 10 } ] } ],
    "SubagentStop":       [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py SubagentStop", "timeout": 10 } ] } ],
    "TaskCreated":        [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py TaskCreated", "timeout": 10 } ] } ],
    "TaskCompleted":      [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py TaskCompleted", "timeout": 10 } ] } ],
    "TeammateIdle":       [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py TeammateIdle", "timeout": 10 } ] } ],
    "Notification":       [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py Notification", "timeout": 10 } ] } ],
    "ConfigChange":       [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py ConfigChange", "timeout": 10 } ] } ],
    "CwdChanged":         [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py CwdChanged", "timeout": 10 } ] } ],
    "FileChanged":        [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py FileChanged", "timeout": 10 } ] } ],
    "PreCompact":         [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py PreCompact", "timeout": 10 } ] } ],
    "PostCompact":        [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py PostCompact", "timeout": 10 } ] } ],
    "Elicitation":        [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py Elicitation", "timeout": 10 } ] } ],
    "ElicitationResult":  [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py ElicitationResult", "timeout": 10 } ] } ],
    "WorktreeRemove":     [ { "hooks": [ { "type": "command", "command": "$wrapper akto-hooks.py WorktreeRemove", "timeout": 10 } ] } ]
  }
}
EOF
)

    if [ -f "$CLAUDE_SETTINGS_FILE" ]; then
        if command -v jq >/dev/null 2>&1; then
            jq --argjson obs "$obs_hooks_config" '
                .hooks.SessionStart       = $obs.hooks.SessionStart |
                .hooks.SessionEnd         = $obs.hooks.SessionEnd |
                .hooks.InstructionsLoaded = $obs.hooks.InstructionsLoaded |
                .hooks.PermissionRequest  = $obs.hooks.PermissionRequest |
                .hooks.PermissionDenied   = $obs.hooks.PermissionDenied |
                .hooks.PostToolUseFailure = $obs.hooks.PostToolUseFailure |
                .hooks.StopFailure        = $obs.hooks.StopFailure |
                .hooks.SubagentStart      = $obs.hooks.SubagentStart |
                .hooks.SubagentStop       = $obs.hooks.SubagentStop |
                .hooks.TaskCreated        = $obs.hooks.TaskCreated |
                .hooks.TaskCompleted      = $obs.hooks.TaskCompleted |
                .hooks.TeammateIdle       = $obs.hooks.TeammateIdle |
                .hooks.Notification       = $obs.hooks.Notification |
                .hooks.ConfigChange       = $obs.hooks.ConfigChange |
                .hooks.CwdChanged         = $obs.hooks.CwdChanged |
                .hooks.FileChanged        = $obs.hooks.FileChanged |
                .hooks.PreCompact         = $obs.hooks.PreCompact |
                .hooks.PostCompact        = $obs.hooks.PostCompact |
                .hooks.Elicitation        = $obs.hooks.Elicitation |
                .hooks.ElicitationResult  = $obs.hooks.ElicitationResult |
                .hooks.WorktreeRemove     = $obs.hooks.WorktreeRemove
            ' "$CLAUDE_SETTINGS_FILE" > "$CLAUDE_SETTINGS_FILE.tmp"
            mv "$CLAUDE_SETTINGS_FILE.tmp" "$CLAUDE_SETTINGS_FILE"
            log "Merged observability hooks into settings.json"
        else
            log "jq not available — skipping observability hooks merge"
        fi
    else
        echo "$obs_hooks_config" > "$CLAUDE_SETTINGS_FILE"
        log "Created settings.json with observability hooks"
    fi
}

main() {
    log "Starting Claude CLI hook installation..."
    log "Target user home: $TARGET_USER_HOME"

    if ! check_claude_installed; then
        log "Claude CLI not detected - skipping hook installation"
        return 0
    fi

    log "Claude CLI detected"

    INGESTION_URL=$(get_ingestion_url)
    if [ -z "$INGESTION_URL" ]; then
        log "Warning: AKTO_DATA_INGESTION_URL not configured"
        INGESTION_URL="https://your-ingestion-url.akto.io"
    fi

    API_TOKEN=$(get_api_token)

    DEVICE_ID=$(generate_device_id)
    if [ -z "$DEVICE_ID" ]; then
        log "Warning: Could not generate device ID"
        DEVICE_ID="unknown-device"
    fi

    log "Device ID: $DEVICE_ID"

    mkdir -p "$CLAUDE_HOOKS_DIR"
    log "Created hooks directory: $CLAUDE_HOOKS_DIR"

    # Get real user (works on both macOS and Linux)
    REAL_USER=""
    if [ "$IS_MACOS" = true ]; then
        REAL_USER="$(stat -f '%Su' "$TARGET_USER_HOME" 2>/dev/null || echo "")"
    else
        REAL_USER="$(stat -c '%U' "$TARGET_USER_HOME" 2>/dev/null || echo "")"
    fi

    if [ -n "$REAL_USER" ] && [ "$REAL_USER" != "root" ]; then
        chown -R "$REAL_USER" "$TARGET_USER_HOME/.claude" 2>/dev/null || true
        log "Set ownership to $REAL_USER"
    fi

    log "Downloading hook scripts from GitHub..."

    if ! download_file "$GITHUB_SHARED_BASE/akto_ingestion_utility.py" "$CLAUDE_HOOKS_DIR/akto_ingestion_utility.py"; then
        log_error "Failed to download akto_ingestion_utility.py"
        return 1
    fi
    chmod +x "$CLAUDE_HOOKS_DIR/akto_ingestion_utility.py"
    log "Downloaded akto_ingestion_utility.py"

    if ! download_file "$GITHUB_RAW_BASE/akto_machine_id.py" "$CLAUDE_HOOKS_DIR/akto_machine_id.py"; then
        log_error "Failed to download akto_machine_id.py"
        return 1
    fi
    chmod +x "$CLAUDE_HOOKS_DIR/akto_machine_id.py"
    log "Downloaded akto_machine_id.py"

    HOOK_PY_FILES=(
        "akto-hooks.py"
    )

    for f in "${HOOK_PY_FILES[@]}"; do
        if ! download_file "$GITHUB_RAW_BASE/$f" "$CLAUDE_HOOKS_DIR/$f"; then
            log_error "Failed to download $f"
            return 1
        fi
        chmod +x "$CLAUDE_HOOKS_DIR/$f"
        log "Downloaded $f"
    done

    create_hook_wrapper "$INGESTION_URL" "$DEVICE_ID"

    log "Installing prompt hooks (UserPromptSubmit/Stop)..."

    if ! download_file "$GITHUB_RAW_BASE/akto-validate-prompt.py" "$CLAUDE_HOOKS_DIR/akto-validate-prompt.py"; then
        log_error "Failed to download akto-validate-prompt.py"
        return 1
    fi
    log "Downloaded akto-validate-prompt.py"

    if ! download_file "$GITHUB_RAW_BASE/akto-validate-response.py" "$CLAUDE_HOOKS_DIR/akto-validate-response.py"; then
        log_error "Failed to download akto-validate-response.py"
        return 1
    fi
    log "Downloaded akto-validate-response.py"

    chmod +x "$CLAUDE_HOOKS_DIR/akto-validate-prompt.py"
    chmod +x "$CLAUDE_HOOKS_DIR/akto-validate-response.py"

    create_prompt_wrapper "prompt" "$INGESTION_URL" "$DEVICE_ID"
    log "Created akto-validate-prompt-wrapper.sh"

    create_prompt_wrapper "response" "$INGESTION_URL" "$DEVICE_ID"
    log "Created akto-validate-response-wrapper.sh"

    update_claude_settings
    log "Updated settings.json with prompt hooks"

    install_mcp_hooks "$INGESTION_URL" "$DEVICE_ID"
    log "MCP hooks installed"

    install_observability_hooks
    log "Observability hooks installed"

    if [ -n "$REAL_USER" ] && [ "$REAL_USER" != "root" ]; then
        chown -R "$REAL_USER" "$TARGET_USER_HOME/.claude" 2>/dev/null || true
    fi

    log ""
    log "=========================================="
    log "Claude CLI hooks installed successfully!"
    log "=========================================="
    log ""
    log "Hooks location: $CLAUDE_HOOKS_DIR"
    log "Settings: $CLAUDE_SETTINGS_FILE"
    log "Data Ingestion URL: $INGESTION_URL"

    return 0
}

if [ -n "$TARGET_USER_HOME" ]; then
    if [ ! -d "$TARGET_USER_HOME" ]; then
        log_error "TARGET_USER_HOME is not a valid directory: $TARGET_USER_HOME"
        exit 1
    fi
    install_for_user "$TARGET_USER_HOME"
    exit $?
else
    EXIT_CODE=0

    # Glob directly in the for-clause (not through an unquoted variable) so this expands
    # correctly under both bash and zsh — CrowdStrike RTR executes -CloudFile scripts under
    # zsh regardless of the #!/bin/bash shebang, and zsh does not glob unquoted variable
    # expansion by default (no GLOB_SUBST), so `for u in $USER_DIRS` would silently iterate
    # once over the literal pattern string instead of real directories.
    if [ "$IS_MACOS" = true ]; then
        GLOB_BASE="/Users"
    else
        GLOB_BASE="/home"
    fi

    for u in "$GLOB_BASE"/*; do
        u="${u%/}"
        base="$(basename "$u")"
        [ "$base" = "Shared" ] && continue
        [ "$base" = "Guest" ] && continue
        case "$base" in .*) continue ;; esac
        [ -d "$u" ] || continue

        log "=== Processing user: $u ==="
        install_for_user "$u" || EXIT_CODE=1
    done
    exit $EXIT_CODE
fi
