...
app = FastAPI(title="Toxic Speech Detection Service", version="1.0.0")
scanner_cache = {}
...
def get_scanner(scanner_type: str, scanner_name: str, config: Dict[str, Any]):
    ...
    if scanner_type == "prompt":
        scanner_class = INPUT_SCANNERS.get(scanner_name)
...
async def health_check():
    return {"status": "healthy", "service": "toxic-speech-detection"}

@app.get("/scanners")
async def list_scanners():
    return {
        "input_scanners": list(INPUT_SCANNERS.keys()),
