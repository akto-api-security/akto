...
public final class McpRegistryUtils {
    ...
    /**
     * Check if an MCP server is listed in the official MCP Registry and log the result.
     * 
     * <p>This is a convenience method that checks registry availability and logs the result.
     * It does not update any database - that should be handled by the caller if needed.</p>
     * 
     * @param hostname The hostname/domain of the server (e.g., "mcp.razorpay.com", without protocol)
     * @param displayName The display name of the collection/server for logging purposes
     * @return "available" if the server is found in the registry, null otherwise
     */
    public static String checkAndLogRegistryStatus(String hostname, String displayName) {
        if (hostname == null || hostname.trim().isEmpty()) {
            logger.debug("checkAndLogRegistryStatus called with null or empty hostname");
            return null;
        }
        
        try {
            logger.debug("Checking MCP Registry for hostname: {}", hostname);
            
            String registryStatus = checkRegistryAvailability(hostname);
            
            if (registryStatus != null && registryStatus.equals(STATUS_AVAILABLE)) {
                logger.info("✓ MCP server '{}' (collection: {}) is LISTED in official MCP Registry", 
                    hostname, displayName);
            } else {
                logger.info("✗ MCP server '{}' (collection: {}) is NOT listed in MCP Registry", 
                    hostname, displayName);
            }
            ...
            logger.error("Error checking MCP registry for hostname: {}", hostname, e);
            return null;
        }
    }
