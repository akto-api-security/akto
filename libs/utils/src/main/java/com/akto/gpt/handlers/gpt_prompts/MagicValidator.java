    ...
    protected String getPrompt(BasicDBObject queryData) {
        return
            "You are a strict security validation engine. "
                ...
                + "   {\"vulnerable\": true, \"reason\": \"<Proof of your finding from the RESPONSE ONLY showing the match>\"}\n"
                + "4. If the RESPONSE does not match the CONTEXT condition, output:\n"
                + "   {\"vulnerable\": false, \"reason\": \"<Proof of your finding from the RESPONSE ONLY showing no match>\"}\n"
                + "5. The 'reason' must always be fact-based, directly derived from the RESPONSE ONLY and relevant to the CONTEXT. Ensure that the 'reason' is a valid json string. It should not give parsing error\n"
