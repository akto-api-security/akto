    ...
    protected String getPrompt(BasicDBObject queryData) {
        ...
        return
            "You are a security risk analyst scoring one AI agent. Everything under \"AGENT DATA\"\n"
                ...
                + "   of what kind of thing it is. Unknown tools contribute NOTHING, positive or\n"
                + "   negative - do not assign them a default capability level, high or low."
                + "\n"
                ...
                + "+0.5  any tool touches highly sensitive data AND any tool has external\n"
                + "      egress (both must be established; internal-only data or no-egress\n"
                + "      setups = +0)\n"
                + "\n"
