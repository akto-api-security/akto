    ...
    protected String getPrompt(BasicDBObject queryData) {
        ...
        return
            "You are a security risk analyst scoring one AI agent. Everything under \"AGENT DATA\" "
                ...
                + "assessed at all.\n\n"
                + "WHAT TO EXTRACT (identify these facts only - no scoring judgment yet; find them "
                + "under whatever field names the data actually uses, don't require an exact "
                + "match):\n"
                + "- Per tool: its specific action (if nameable), capability level (read-only / "
                + "create-modify / delete-admin-financial), data sensitivity it touches (none / "
                + "internal / highly sensitive), and whether it can move data outside the org "
                + "(external egress) or is strictly internal-only. If you recognize the tool's "
                + "underlying platform/brand (e.g. Pipedream, Zapier, Make, n8n, or any generic "
                + "\"run code\"/\"webhook\"/\"HTTP request\" tool), use that category's known ceiling "
                + "capability even without seeing its exact configured action: high capability "
                + "(arbitrary external API calls or code execution) and external egress.\n"
                + "- Agent autonomy: does it decide its own actions dynamically, or follow a "
                + "fixed/scripted path.\n"
                + "- Agent identity: runs under the calling user's own scoped permissions, or a "
                + "broader app/service identity not tied to one person.\n"
                + "- Agent sharing: held by just the owner, or exposed to a wider group / whole "
                + "organization.\n"
                + "- Agent surface: any external-facing publish point (public endpoint, bot, "
                + "widget) beyond direct invocation, or none.\n"
                + "- Agent auth: only flag as unauthenticated if the field is explicitly present "
                + "AND its value clearly reads as no authentication (e.g. \"None,\" \"none,\" "
                + "\"anonymous,\" explicitly blank). If it names any real identity/auth provider, "
                + "treat as authenticated - which provider doesn't matter. If the field is absent / "
                + "not reported at all, that's missing data, not a signal - never treat absent the "
                + "same as unauthenticated.\n"
                + "- UNKNOWN TOOLS: a tool is unknown only if you cannot recognize its brand or "
                + "category at all - no signal on what kind of thing it even is. If you recognize a "
                + "general-purpose automation/integration/scripting platform, score it via the "
                + "category ceiling above instead of calling it unknown. Never use hedging words "
                + "(\"plausible,\" \"could,\" \"may allow\") about a truly unknown tool to raise the "
                + "score - that's inventing risk. Unknown tools contribute nothing, positive or "
                + "negative. Use -1 only if nothing about the agent can be assessed at all.\n\n"
                + "HOW TO SCORE (this is the only place scoring logic lives):\n"
                + "- Start from the riskiest tool with a nameable action or a known category "
                + "ceiling. Truly unknown tools contribute nothing, positive or negative - never "
                + "use hedging language (\"plausible,\" \"could,\" \"may allow\") to raise the score "
                + "from one.\n"
                + "- All assessable tools low-capability/low-sensitivity, no private-data access "
                + "→ 0. Same, but touching the owner's own private data with zero egress and "
                + "narrow sharing → 0.5.\n"
                + "- Raise for a real combo: sensitive-data access + genuine external egress + a "
                + "broad external-facing surface, together. Purely internal use stays low "
                + "regardless of tool count.\n"
                + "- Raise for: sharing beyond the owner, a broad non-user-scoped identity, "
                + "explicit unauthenticated status, an external publish surface. Dynamic/autonomous "
                + "behavior only raises the score when combined with high-capability tools, never by "
                + "itself.\n"
                + "- Missing/empty facts (other than explicit unauthenticated) = no signal, not a "
                + "risk factor either way.\n"
                + "- Same tools + same setup → same score, on any agent, any vendor, any "
                + "schema.\n\n"
