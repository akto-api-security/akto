...
public abstract class ATicketIntegrationService<T> {
    ...
    public final TicketCreationResult createTickets(List<TestingIssuesId> testingIssuesIdList, String projectIdentifier, String workItemType, String aktoDashboardHost)
        throws Exception {
        ...
        Map<TestingIssuesId, TestingRunIssues> allIssuesMap = new HashMap<>();
        if (allIssues != null) {
            for (TestingRunIssues issue : allIssues) {
                allIssuesMap.put(issue.getId(), issue);
                if (StringUtils.isNotBlank(getExistingTicketUrl(issue))) {
                    existingCount++;
                }
            }
        }
        ...
        for (TestingIssuesId issueId : testingIssuesIdList) {
            try {
                TestingRunIssues issue = allIssuesMap.get(issueId);

                if (issue != null && StringUtils.isNotBlank(getExistingTicketUrl(issue))) {
                ...
                GlobalEnums.Severity severity = issue != null ?
                    issue.getSeverity() : GlobalEnums.Severity.HIGH;
