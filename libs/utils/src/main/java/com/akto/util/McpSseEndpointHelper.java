...
public class McpSseEndpointHelper {
     ...
     */
    public static void addSseEndpointHeader(OriginalHttpRequest request, ApiCollection apiCollection) {
        if (request == null) {
            return;
        }
        ...
        try {
            ...
            loggerMaker.warn("Failed to add SSE endpoint header for collection {}: {}", apiCollection.getId(), e.getMessage());
        }
    }
} 