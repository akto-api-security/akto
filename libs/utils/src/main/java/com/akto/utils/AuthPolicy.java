...
public class AuthPolicy {
    ...
    public static Set<ApiInfo.AuthType> findAuthTypes(Map<String, List<String>> headers) {
        ...
        // Check headers for Bearer, Basic, API_KEY, MTLS, SESSION_TOKEN
        for (String header : headers.keySet()) {
            String headerLower = header.toLowerCase();
            List<String> headerValues = headers.getOrDefault(header, new ArrayList<>());
            for (String value : headerValues) {
                authTypes.addAll(findBearerBasicAuth(headerLower, value));
        ...
        // Check cookies for JWT and session tokens
        ...
        // Find JWT in non-Authorization headers
     ...
     * Returns the token without the "Bearer " prefix, or null if not found.
     */
    public static String extractBearerToken(Map<String, List<String>> headers) {
        if (headers == null) return null;

        for (String headerName : headers.keySet()) {
            if (headerName.equalsIgnoreCase(AUTHORIZATION_HEADER_NAME)) {
                List<String> authValues = headers.get(headerName);
                if (authValues == null) continue;
                for (String value : authValues) {
                    if (value != null && value.toLowerCase().trim().startsWith("bearer ")) {
    ...
    /**
     * Extract all JWT tokens from headers and cookies.
     * Returns a list of JWT tokens found.
     */
    public static List<String> extractAllJwtTokens(Map<String, List<String>> headers) {
        List<String> jwtTokens = new ArrayList<>();
        if (headers == null) return jwtTokens;

        // Check Authorization header for Bearer JWT
        String bearerToken = extractBearerToken(headers);
        if (bearerToken != null && KeyTypes.isJWT(bearerToken)) {
            jwtTokens.add(bearerToken);
        }

        // Check cookies for JWT
        List<String> cookieList = headers.getOrDefault(COOKIE_NAME, new ArrayList<>());
        Map<String, String> cookieMap = parseCookie(cookieList);
        for (String cookieValue : cookieMap.values()) {
            if (KeyTypes.isJWT(cookieValue)) {
                jwtTokens.add(cookieValue);
            }
        }

        // Check other headers for JWT
        for (String headerName : headers.keySet()) {
            if (headerName.equalsIgnoreCase(AUTHORIZATION_HEADER_NAME)) {
                continue;
            }
