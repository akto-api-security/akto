...
public class TestingRunResultDao extends AccountsContextDaoWithRbac<TestingRunResult> {
        ...
        public static Bson generateFilter(ObjectId testRunId, ApiInfo.ApiInfoKey apiInfoKey) {
                return generateFilter(testRunId, apiInfoKey.getApiCollectionId(), apiInfoKey.getUrl(),
        ...
        public static Bson generateFilter(ObjectId testRunId, int apiCollectionId, String url, String method) {
                return Filters.and(
                                ...
                                Filters.eq(TestingRunResult.API_INFO_KEY + "." + ApiInfo.ApiInfoKey.API_COLLECTION_ID,
                                                apiCollectionId),
        ...
        public List<TestingRunResult> fetchLatestTestingRunResultWithCustomAggregations(Bson filters, int limit,
                ...
                Bson projections = Projections.fields(
                                Projections.computed("confidence",
                                                Projections.computed("$first", "$testResults.confidence")),
                                getLatestTestingRunResultProjections());
                ...
                return fetchLatestTestingRunResult(filters, limit, skip,
                                Arrays.asList(Aggregates.project(projections), addSeverityValueStage, customSort));
        }

        public List<TestingRunResult> fetchLatestTestingRunResult(Bson filters, int limit, int skip,
                        List<Bson> customAggregation) {
                ...
                while (cursor.hasNext()) {
                        ...
                        try {
                                ...
                                testingRunResult.setTestRunId(
                                                new ObjectId(doc.getString(TestingRunResult.TEST_RUN_ID)));
                                BasicDBObject apiInfoKeyObj = (BasicDBObject) doc.get(TestingRunResult.API_INFO_KEY);
                                ApiInfoKey apiInfoKey = new ApiInfoKey(
                                                apiInfoKeyObj.getInt(ApiInfoKey.API_COLLECTION_ID),
                                ...
                                testingRunResult.setConfidencePercentage(
                                                doc.getInt(TestingRunResult.CONFIDENCE_PERCENTAGE));
                                ...
                                if (testResultsList != null && !testResultsList.isEmpty()) {
                                        ...
                                        if (genericTestResult.get(GenericTestResult._CONFIDENCE) != null) {
                                                ...
                                                try {
                                                        ...
                                                        message = genericTestResult.getString(TestResult._MESSAGE,
                                                                        null);
        ...
        public Map<ObjectId, String> mapSummaryIdToTestingResultHexId(Set<String> testingRunResultHexIds) {
                ...
                List<TestingRunResult> runResults = this.findAll(Filters.in(Constants.ID, objectIdList),
        ...
        public void createIndicesIfAbsent() {
                ...
                if (DbMode.allowCappedCollections()) {
                        createCollectionOptions = new CreateCollectionOptions().capped(true).maxDocuments(maxDocuments)
                                        .sizeInBytes(sizeInBytes);
                }
                createCollectionIfAbsent(dbName, getCollName(), createCollectionOptions);

                Bson summaryIndex = Indexes
                                .descending(Arrays.asList(TestingRunResult.TEST_RUN_RESULT_SUMMARY_ID, Constants.ID));
                createIndexIfAbsent(dbName, getCollName(), summaryIndex,
                                new IndexOptions().name("testRunResultSummaryId_-1__id_-1"));

                MCollection.createIndexIfAbsent(getDBName(), getCollName(),
                                new String[] { TestingRunResult.TEST_RUN_RESULT_SUMMARY_ID, TestingRunResult.VULNERABLE,
                                                Constants.ID },
                                false);
                ...
                fieldNames = new String[] { TestingRunResult.TEST_RUN_RESULT_SUMMARY_ID,
                                TestingRunResult.API_INFO_KEY + "." + ApiInfoKey.API_COLLECTION_ID };
                MCollection.createIndexIfAbsent(getDBName(), getCollName(), fieldNames, false);
                ...
                fieldNames = new String[] { TestingRunResult.TEST_RUN_RESULT_SUMMARY_ID,
                                TestingRunResult.API_INFO_KEY + "." + ApiInfoKey.METHOD };
                MCollection.createIndexIfAbsent(getDBName(), getCollName(), fieldNames, false);
        ...
        public void convertToCappedCollection() {
                if (DbMode.allowCappedCollections() || this.isCapped())
                        return;
